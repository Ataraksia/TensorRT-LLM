...
def get_build_dir(build_dir, build_type):
    if build_dir is None:
        build_dir = get_source_dir() / (
            "build" if build_type == "Release" else f"build_{build_type}"
        )
    else:
...
def setup_venv(
    project_dir: Path, requirements_file: Path, no_venv: bool
) -> tuple[Path, Path]:
    ...
    if no_venv or sys.prefix != sys.base_prefix:
        reason = (
            ...
            if no_venv
            else "Already inside virtual environment"
        )
    ...
    if os.environ.get("NVIDIA_PYTORCH_VERSION"):
        ...
        try:
            info = check_output([str(venv_python), "-m", "pip", "show", "torch"])
    ...
    print(f"-- Installing requirements from {requirements_file} into {venv_prefix}...")
    ...
    if not venv_conan.exists():
        ...
        try:
            ...
            if conan_path_str:
                ...
                print(f"-- Found conan executable via PATH search at: {venv_conan}")
            else:
                raise RuntimeError(
            ...
            print(f"Fallback search command output: {e.stdout}", file=sys.stderr)
...
def generate_fmha_cu(project_dir, venv_python):
    ...
    cubin_dir = (
        project_dir / "cpp/tensorrt_llm/kernels/contextFusedMultiHeadAttention/cubin"
    )
    move_if_updated(fmha_v2_dir / "generated/fmha_cubin.h", cubin_dir / "fmha_cubin.h")
...
def create_cuda_stub_links(cuda_stub_dir: str, missing_libs: list[str]) -> str:
    ...
    if not cuda_stub_path.exists():
        raise RuntimeError(f"CUDA stub directory '{cuda_stub_dir}' does not exist.")
    ...
    version_pattern = r"\.\d+"
    for missing_lib in filter(lambda x: re.search(version_pattern, x), missing_libs):
...
def generate_python_stubs_linux(
    ...
    cuda_home_dir = (
        env_stub_gen.get("CUDA_HOME")
        or env_stub_gen.get("CUDA_PATH")
        or "/usr/local/cuda"
    )
...
def generate_python_stubs_windows(
    binding_type: str, venv_python: Path, pkg_dir: Path, lib_dir: Path
):
...
def main(
    *,
    ...
    with open(project_dir / ".gitmodules", "r") as submodules_f:
        submodules = [
            l.split("=")[1].strip() for l in submodules_f.readlines() if "path = " in l
        ]
    ...
    venv_python, venv_conan = setup_venv(
        project_dir, project_dir / requirements_filename, no_venv
    )
    ...
    install_tree(
        get_source_dir() / "include" / "tensorrt_llm" / "deep_gemm",
    ...
    if cuda_include_dir is None or not cuda_include_dir.exists():
        print("CUDA_HOME or CUDA_PATH should be set to enable DeepGEMM JIT compilation")
    else:
    ...
    if on_windows:
        install_file(
            build_dir / "tensorrt_llm/tensorrt_llm.dll", lib_dir / "tensorrt_llm.dll"
        )
        install_file(
            build_dir / f"tensorrt_llm/thop/th_common.dll", lib_dir / "th_common.dll"
        )
        install_file(
        ...
        install_file(
            build_dir / "tensorrt_llm/libtensorrt_llm.so",
            lib_dir / "libtensorrt_llm.so",
        )
        install_file(
            build_dir / "tensorrt_llm/thop/libth_common.so", lib_dir / "libth_common.so"
        )
        install_file(
    ...
    if not on_windows:
        install_file(
            build_dir / "tensorrt_llm/executor_worker/executorWorker",
    ...
    if not cpp_only:
        ...
        def get_binding_lib(subdirectory, name):
            ...
            assert (
                len(binding_lib) == 1
            ), f"Exactly one binding library should be present: {binding_lib}"
        ...
        if not skip_stubs:
            with working_directory(pkg_dir):
                if on_windows:
                    generate_python_stubs_windows(
                        binding_type, venv_python, pkg_dir, lib_dir
                    )
                else:  # on linux
            ...
            if os.environ.get("CUDA_VERSION", "").startswith("12."):
                ...
                while i < len(lines):
                    ...
                    if "<For CUDA 12.9>" in line and line.strip().startswith("#"):
                        ...
                        print(f"Enable CUDA 12.9 dependency: {new_line.strip()}")
                        modified_lines.append(new_line)
                        print(f"Disable CUDA 13 dependency: # {lines[i + 1].strip()}")
...
def add_arguments(parser: ArgumentParser):
    ...
    parser.add_argument(
        "--cuda_architectures",
        "-a",
        help="CUDA architectures to build for, will be passed to cmake `CUDA_ARCHITECTURES` variable. Example: `--cuda_architectures=90-real;100-real`",
    )
    parser.add_argument(
        "--install",
        "-i",
    ...
    parser.add_argument(
        "--clean_wheel",
        action="store_true",
        help="Clear dist_dir folder when creating wheel. Will be set to `true` if `--clean` is set",
    )
    parser.add_argument(
        "--configure_cmake",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "-D",
        action="append",
        help='Extra cmake variable definitions which can be specified multiple times. Example: -D "key1=value1" -D "key2=value2"',
        default=[],
    )
    parser.add_argument(
        "--extra-make-targets",
    ...
    parser.add_argument(
        "--nccl_root", help="Directory containing NCCL headers and libraries"
    )
    parser.add_argument(
        "--nixl_root", help="Directory containing NIXL headers and libraries"
    )
    parser.add_argument(
        "--internal-cutlass-kernels-root",
        default="",
        help="Directory containing internal_cutlass_kernels sources. If specified, the internal_cutlass_kernels and NVRTC wrapper libraries will be built from source.",
    )
    parser.add_argument(
        "--build_dir",
        type=Path,
        help="Directory where C++ sources are built (default: cpp/build or cpp/build_<build_type>)",
    )
    parser.add_argument(
        "--dist_dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        "-s",
        action="store_true",
        help="Skip building the *.whl files (they are only needed for distribution)",
    )
    parser.add_argument(
        "--linking_install_binary",
        action="store_true",
        help="Install the built binary by creating symbolic links instead of copying files",
    )
    parser.add_argument(
        "--binding_type",
    ...
    parser.add_argument(
        "--nvtx", action="store_true", help="Enable NVTX profiling features"
    )
    parser.add_argument(
        "--skip-stubs", action="store_true", help="Skip building Python type stubs"
    )
    parser.add_argument(
        "--generate_fmha", action="store_true", help="Generate the FMHA CUDA files"
    )
    parser.add_argument(
        "--no-venv",
        action="store_true",
