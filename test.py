...
def _build_delay_pattern_mask(
    input_ids: torch.LongTensor, bos_token_id: int, pad_token_id: int
):
...
def revert_delay_pattern(data):
    ...
    return (
        torch.cat(out_l, dim=0)
        if isinstance(data, torch.Tensor)
        else np.concatenate(out_l, axis=0)
    )
...
class HiggsAudioTokenizer(nn.Module):
    ...
    def _xcodec_encode(
        self, x: torch.Tensor, target_bw: int | None = None
    ) -> torch.Tensor:
        bw = target_bw
        ...
        with torch.no_grad():
            ...
            for i in range(0, codes.shape[-1], chunk_size):
                ...
                if i == 0:
                    ...
                    last_chunk_output_length = self.xcodec_get_output_length(
                        last_chunk_size
                    )
...
class HiggsAudioInfer:
    ...
    def __init__(
        ...
        self.reference_audio = (
            None  # Disable reference audio loading for faster testing
        )
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.hf_model_dir, trust_remote_code=True
        )
        ...
        if self.reference_audio and self.audio_tokenizer:
            ...
            if not os.path.exists(self.reference_audio):
                raise FileNotFoundError(
                    f"Reference audio file not found: {self.reference_audio}"
                )
            ...
            post_audio_input = (
                "<|audio_eos|><|eot_id|><|start_header_id|>user<|end_header_id|>\n\n"
            )
            ...
            input_ids = torch.cat(
                [pre_audio_input_ids, audio_ids, post_audio_input_ids]
            )
            self.config.audio_in_start = pre_audio_input_ids.shape[0]
            self.config.audio_in_end = (
                pre_audio_input_ids.shape[0] + audio_ids.shape[0] - 1
            )
            np.savetxt(
                "audio_ids_in.txt", audio_ids.cpu().view(8, -1), delimiter=",", fmt="%d"
            )
        else:
            ...
            input_ids = (
                self.tokenizer.encode(text_input, return_tensors="pt")
                .to(self.device)
                .flatten()
            )
    ...
    def generate(
        ...
        **generation_kwargs,
        ...
        audio_out_bos_token = torch.full(
        ...
        input_ids = torch.cat([self.saved_input_ids, input_ids, audio_out_bos_token])
        max_new_tokens = self.config.max_num_tokens - input_ids.shape[0]
        self.config.audio_out_start = input_ids.shape[0] - audio_out_bos_token.shape[0]
        with torch.no_grad():
        ...
        print(
            f"Global token range: min={audio_ids.min().item()}, max={audio_ids.max().item()}"
        )
        ...
        audio_ids = revert_delay_pattern(
            audio_ids.view(self.config.num_codebooks, -1)
        ).clip(0, self.config.codebook_size - 2 - 1)
        ...
        if sr != target_sr:
            ...
            wv_np = (
                waveform if isinstance(waveform, np.ndarray) else waveform.cpu().numpy()
            )
