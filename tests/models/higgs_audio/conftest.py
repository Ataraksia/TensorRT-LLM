...
class MockTensor:
    ...
    def contiguous(self):
        return MockTensor(np.ascontiguousarray(self.data), self.dtype, self.device, self.requires_grad)
...
def performance_benchmarker():
    ...
    class PerformanceBenchmarker:
        ...
        def validate_improvement(self, baseline_metric, optimized_metric, 
                               expected_improvement_pct, tolerance_pct=5):
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if "generation_mode" in test_name or "GenerationMode" in (item.cls.__name__ if item.cls else ""):
...
def pytest_addoption(parser):
    """Add custom command line options for TTS testing."""
    parser.addoption("--run-integration", action="store_true", default=False,
                     help="run integration tests")
    parser.addoption("--run-performance", action="store_true", default=False,
                     help="run performance validation tests")
    parser.addoption("--run-end-to-end", action="store_true", default=False,
                     help="run end-to-end pipeline tests")
    parser.addoption("--run-slow", action="store_true", default=False,
                     help="run slow tests")
    parser.addoption("--run-cuda-graphs", action="store_true", default=False,
                     help="run CUDA graph optimization tests")


def pytest_runtest_setup(item):
    """Skip tests based on command line options and availability."""
