...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers",
        "real_world: marks tests as real-world scenario tests",
    )
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--run-real-world", action="store_true", default=False, help="run real-world scenario tests"
    )
    parser.addoption(
        "--run-performance",
        action="store_true",
    ...
    parser.addoption(
        "--run-end-to-end", action="store_true", default=False, help="run end-to-end pipeline tests"
    )
    parser.addoption(
        "--run-cuda-graphs",
        action="store_true",
...
def pytest_runtest_setup(item):
    ...
    if "end_to_end" in item.keywords and not item.config.getoption("--run-end-to-end"):
        pytest.skip("need --run-end-to-end option to run end-to-end tests")

    if "cuda_graphs" in item.keywords and not item.config.getoption("--run-cuda-graphs"):
        pytest.skip("need --run-cuda-graphs option to run cuda-graphs tests")
