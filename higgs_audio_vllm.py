...
from typing import Any, ClassVar, List, Literal, Optional, Set, Tuple, TypedDict, Union
...
def revert_delay_pattern(data):
    ...
    for i in range(num_codebooks):
        out_l.append(
            data[i : (i + 1), i : (data.shape[1] - num_codebooks + 1 + i)]
        )
...
def load_higgs_audio_tokenizer(tokenizer_name_or_path, device="cuda"):
    ...
    if os.path.exists(config_path):
        ...
        config = OmegaConf.load(
            os.path.join(tokenizer_path, "config.yaml")
        ).generator.config
    else:
...
class HiggsAudioDualFFNDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if rope_scaling is not None and getattr(text_config, "original_max_position_embeddings", None):
            rope_scaling["original_max_position_embeddings"] = text_config.original_max_position_embeddings
