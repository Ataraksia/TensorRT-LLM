...
class HiggsAudioModelRunner(ModelRunnerCpp):
    ...
    def _apply_delay_pattern_to_full_sequence(self, tokens):
        ...
        self.debug_log.append(f"Audio section: {len(audio_tokens)} tokens from {audio_tokens_start} to {audio_end_idx}")
        ...
        if len(audio_tokens) == 0 or len(audio_tokens) % self.config.num_codebooks != 0:
            ...
            if len(audio_tokens) >= self.config.num_codebooks:
                complete_tokens = (len(audio_tokens) // self.config.num_codebooks) * self.config.num_codebooks
                self.debug_log.append(f"Truncating {len(audio_tokens)} audio tokens to {complete_tokens} complete frames")
                audio_tokens = audio_tokens[:complete_tokens]
                audio_end_idx = audio_tokens_start + len(audio_tokens)
            else:
                self.debug_log.append(f"Audio tokens not valid for delay pattern: {len(audio_tokens)} % {self.config.num_codebooks} = {len(audio_tokens) % self.config.num_codebooks}")
                return tokens
        ...
        self.debug_log.append(f"Reshaping to {num_frames} frames x {self.config.num_codebooks} codebooks")
        ...
        self.debug_log.append(f"Expected {expected_modifications} modifications, made {modifications_count}")
        ...
        for frame_idx in [0, 1, 2, 7, 50, 100]:
            if frame_idx < num_frames:
                ...
                self.debug_log.append(f"Frame {frame_idx} (active_cbs={active_cbs}): BOS={bos_count}, content={content_count}, tokens={frame_tokens}")
        
        if modified:
            # Replace audio section in original tokens
            modified_audio_tokens = audio_tensor.flatten().tolist()
            modified_tokens = tokens[:audio_tokens_start] + modified_audio_tokens + tokens[audio_end_idx:]
            
            # Final verification: check delay pattern is correct
            final_tensor = torch.tensor(modified_audio_tokens).view(num_frames, self.config.num_codebooks)
            ...
            self.debug_log.append(f"✅ DELAY PATTERN VERIFICATION: {'PASSED' if delay_pattern_correct else 'FAILED'}")
