...
class ModelRunnerCpp(ModelRunnerMixin):
    ...
    def from_dir(
        ...
        *,
        ...
        if (
            ...
            if multi_block_mode is not None:
                logger.warning(f"Multi block mode is not supported for EAGLE. Disabling it.")
        ...
        trtllm_config.fail_fast_on_attention_window_too_large = (
        ...
        if is_enc_dec:
            ...
            executor = trtllm.Executor(
                Path(engine_dir), trtllm.ModelType.DECODER_ONLY, trtllm_config
            )
    ...
    def generate(
        ...
        **kwargs,
        ...
        stop_words_list = self._prepare_words_list(stop_words_list, len(batch_input_ids_list))
        bad_words_list = self._prepare_words_list(bad_words_list, len(batch_input_ids_list))
    ...
    def _prepare_mrope_executor(self, batch_input_ids_list, mrope: MropeParams):
        ...
        if mrope != None:
            ...
            mrope_rotary_cos_sin_data = mrope_rotary_cos_sin.to(dtype=torch.float32)

            mrope_position_deltas = mrope.mrope_position_deltas
            assert isinstance(mrope_position_deltas, torch.Tensor), (
        ...
        *,
        ...
        if return_dict:
            ...
            input_lengths = torch.tensor(
                [x.size(0) for x in batch_input_ids], dtype=torch.int32, device=cuda_device
            )
            ...
            if output_log_probs:
                max_log_probs_len = max(
                    len(lprobs) for response in responses for lprobs in response.result.log_probs
                )
                log_probs = torch.zeros(
                    (batch_size, num_sequences, max_log_probs_len), dtype=torch.float32
                )
                for response in responses:
                    batch_idx = request_ids.index(response.request_id)
                    if is_beam_search:
                        for beam_idx, lprobs in enumerate(response.result.log_probs):
                            log_probs[batch_idx, beam_idx, : len(lprobs)] = torch.tensor(lprobs)
                    else:
                        seq_idx = response.result.sequence_index
                        lprobs = response.result.log_probs[0]
                        log_probs[batch_idx, seq_idx, : len(lprobs)] = torch.tensor(lprobs)
