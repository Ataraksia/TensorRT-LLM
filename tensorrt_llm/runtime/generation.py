...
from .._utils import pad_vocab_size, str_dtype_to_torch, torch_to_numpy, trt_dtype_to_torch
...
def decode_words_list(word_dict: List[List[str]], tokenizer=None, add_special_tokens=False):
    """
    format of word_dict
...
def _prepare_input_ids(tensors: Sequence[torch.Tensor]):
    ...
    row_lengths = torch.tensor(row_lengths, dtype=torch.int32, device=data.device)
...
def _prepare_attention_mask(input_ids: torch.Tensor, pad_id: Optional[int] = None):
    ...
    if input_ids is not None and is_pad_id_in_inputs:
        ...
        return torch.ones(input_ids.shape, dtype=torch.int32, device=input_ids.device)
...
def _contiguous_tile_beam_width(tensor: torch.Tensor, size: int, num_beams: int):
    new_shape = list(tensor.shape)
    new_shape[0] *= num_beams

    numel = tensor.numel()
    new_tensor = torch.empty(num_beams * numel, device=tensor.device, dtype=tensor.dtype)
...
class _Runtime(object):
    ...
    def __create_and_setup_context(
        self, address, size, profile_idx, stream
    ) -> trt.IExecutionContext:
        context = self.engine.create_execution_context_without_device_memory()
        assert context is not None, (
            "Failed to create an execution context with the provided device memory!"
        )
    ...
    def __print_engine_info(self) -> None:
        ...
        for name in tensor_name_list:
            ...
            item["mode"] = "I" if engine.get_tensor_mode(name) == trt.TensorIOMode.INPUT else "O"
            item["location"] = "GPU" if engine.get_tensor_location(name) else "CPU"
                ...
                for name in tid.keys():
                    if tid[name]["mode"] == "I":
                        if tid[name]["location"] == "GPU":
                            context.set_input_shape(name, tid[name]["profile_list"][k][j])
                        else:
                            context.set_tensor_address(
                                name, tid[name]["profile_list"][k][j].ctypes.data
                            )
                    elif tid[name]["mode"] == "O":
                        assert (
                            context.all_binding_shapes_specified
                            and context.all_shape_inputs_specified
                        )
    ...
    def print_context_info(self, context, context_index) -> None:
        ...
        for i in range(n_io):
            name = self.engine.get_tensor_name(i)
            b_input = self.engine.get_tensor_mode(name) == trt.TensorIOMode.INPUT
        ...
        logger.debug(f"{'Name':^{max_name_width}}|I/O|{'Shape':^{max_shape_width}}|")
        logger.debug(f"{'-' * (max_name_width + max_shape_width + 6)}")
        for i in range(n_io):
    ...
    def _set_shape(self, context: trt.IExecutionContext, shape_dict: Dict[str, List[int]]):
        for i in range(self.engine.num_io_tensors):
            name = self.engine.get_tensor_name(i)
    ...
    def _set_buffer(self, context: trt.IExecutionContext, buffer_dict: Dict[str, torch.Tensor]):
        for i in range(self.engine.num_io_tensors):
            name = self.engine.get_tensor_name(i)
            if name not in buffer_dict.keys():
                dtype = self.engine.get_tensor_dtype(name)
                shape = context.get_tensor_shape(name)
                buffer_dict[name] = torch.zeros(
                    tuple(shape), dtype=trt_dtype_to_torch(dtype), device="cuda"
                )
            assert buffer_dict[name].is_contiguous(), f"{name} is not contiguous()"
            context.set_tensor_address(name, buffer_dict[name].data_ptr())

    def _set_tensors(self, context: trt.IExecutionContext, tensors: Dict[str, "RuntimeTensor"]):
        ...
        for name in self.output_tensor_names:
            if name not in tensors:
                ...
                tensors[name] = RuntimeTensor.from_torch(
                    name, torch.zeros(tuple(shape), dtype=trt_dtype_to_torch(dtype), device="cuda")
                )
            t = tensors[name]
    ...
    def _set_weight_streaming(self, gpu_weights_percent):
        ...
        assert self.engine.weight_streaming_budget_v2 == budget, (
    ...
    def _run(
        self, context: trt.IExecutionContext, stream: Union[int, torch.cuda.Stream] = None
    ) -> bool:
        if stream is None:
...
class SamplingConfig:
    ...
    stop_words_list: Optional[Union[list, np.ndarray, torch.Tensor]] = field(default=None)
    bad_words_list: Optional[Union[list, np.ndarray, torch.Tensor]] = field(default=None)
    ...
    beam_search_diversity_rate: Union[float, torch.Tensor] = field(init=False, default=0.0)
    output_cum_log_probs: bool = field(init=False, default=False)
    output_log_probs: bool = field(init=False, default=False)
    no_repeat_ngram_size: Union[int, torch.Tensor] = field(init=False, default=None)
    min_p: Union[float, torch.Tensor] = field(default=0.0)
    tokens_per_step: Optional[int] = field(default=None)
...
class LogitsProcessor:
    ...
    def __call__(self, step: int, input_ids: torch.Tensor, scores: torch.Tensor) -> torch.Tensor:
        raise NotImplementedError(
...
class StoppingCriteria:
    ...
    def __call__(self, step: int, input_ids: torch.Tensor, scores: torch.Tensor) -> bool:
...
class RuntimeTensor:
    ...
    @staticmethod
    def from_pointer(name: str, pointer, shape, str_dtype: str) -> "RuntimeTensor":
        t = RuntimeTensor()
        t._name = name
        t._data_ptr = pointer
        t._shape = shape
    ...
    @staticmethod
    def from_torch(
        name: str, data: torch.Tensor, override_shape: Optional[Iterable] = None
    ) -> "RuntimeTensor":
        ...
        if override_shape is not None:
            ...
            assert isinstance(override_shape, list) or isinstance(override_shape, tuple)
...
class GenerationSession(object):
    ...
    def __init__(
        ...
        if DISABLE_TORCH_DEVICE_SET:
            ...
            self.device = torch.device(f"cuda:{self.runtime.runtime_rank % mapping.gpus_per_node}")
        ...
        self.vocab_size_padded = pad_vocab_size(self.vocab_size, self.mapping.tp_size)
        if len(model_config.layer_types) == 0:
            self.layer_types = ["attention"] * model_config.num_layers
        else:
            layer_types = model_config.layer_types
            layer_types = layer_types * (model_config.num_layers // len(layer_types))
            layer_types = (
                layer_types + layer_types[0 : (model_config.num_layers % len(layer_types))]
            )
            self.layer_types = layer_types
        self.num_attn_layers = self.layer_types[self.first_layer : self.last_layer].count(
            "attention"
        )
        self.has_attn_layers = self.num_attn_layers > 0
        self.has_rnn_layers = "recurrent" in self.layer_types[self.first_layer : self.last_layer]
        ...
        if self.mapping.tp_size > 1:
            self.ipc_buffers, self.all_reduce_workspace = CustomAllReduceHelper.allocate_workspace(
                self.mapping, CustomAllReduceHelper.max_workspace_size_auto(self.mapping.tp_size)
            )
        ...
        if self.paged_kv_cache and self.has_attn_layers:
            ...
            if self.use_kv_cache and not self.paged_kv_cache:
                for i in range(self.first_layer, self.last_layer):
                    if self.layer_types[i] == "attention":
                        expected_tensor_names += [f"past_key_value_{i}", f"present_key_value_{i}"]
        ...
        if self.paged_state and self.has_rnn_layers:
            for i in range(self.first_layer, self.last_layer):
                if self.layer_types[i] == "recurrent":
                    expected_tensor_names += [f"conv_state_ptr_{i}", f"rnn_state_ptr_{i}"]
            ...
            for i in range(self.first_layer, self.last_layer):
                if self.layer_types[i] == "recurrent":
                    expected_tensor_names += [
                        ...
                        f"past_rnn_state_{i}",
                        f"present_rnn_state_{i}",
                    ]

        if model_config.gpt_attention_plugin and self.has_attn_layers:
            if self.use_kv_cache:
                expected_tensor_names += ["sequence_length", "host_past_key_value_lengths"]
        ...
        if model_config.max_prompt_embedding_table_size > 0:
            expected_tensor_names += ["prompt_embedding_table", "tasks", "prompt_vocab_size"]
        ...
        self.missing_qkv_modules = LoraManager.get_missing_qkv_modules(self.lora_target_modules)
        ...
        for name in found_tensor_names:
            if name.startswith("allreduce_ub_") or name.startswith("gemm_allreduce"):
    ...
    def get_num_heads_kv(self, layer_idx: Optional[int] = None) -> int:
        ...
        if self._model_config.layer_types:
            assert self._model_config.layer_types[layer_idx] == "attention", (
                f"Layer {layer_idx} is not an attention layer"
            )
    ...
    def head_size(self):
        return (
            ...
            if self._model_config.head_size is None
            else self._model_config.head_size
        )
    ...
    def is_redrafter_mode(self):
        return (
            self._model_config.redrafter_num_beams > 0
            and self._model_config.redrafter_draft_len_per_beam > 0
        )
    ...
    def max_draft_tokens(self):
        if self.is_redrafter_mode:
            return (
                self._model_config.redrafter_num_beams
    ...
    def _capture_cuda_graph_and_instantiate(self, context, stream, step):
        ...
        if self.runtime.cuda_graph_instances[instance_idx] is not None:
            self.runtime.cuda_graph_instances[instance_idx] = _update_cuda_graph_instance(
                self.runtime.cuda_graph_instances[instance_idx], next_graph
            )
        else:
            self.runtime.cuda_graph_instances[instance_idx] = CUASSERT(
                cudart.cudaGraphInstantiate(next_graph, 0)
            )[0]

        # Pre-upload cuda graph to stream
        CUASSERT(cudart.cudaGraphUpload(self.runtime.cuda_graph_instances[instance_idx], stream))
    ...
    def __setup_decoder(
        ...
        if isinstance(scfg.top_k, torch.Tensor):
            assert scfg.top_k.dtype == torch.int32, (
                f"scfg.top_k.dtype ({scfg.top_k.dtype}) must be torch.int32"
            )
            assert scfg.top_k.shape[0] == batch_size, (
                f"scfg.top_k.shape[0] ({scfg.top_k.shape[0]}) must equal to batch_size ({batch_size})"
            )
        ...
        if isinstance(scfg.top_p, torch.Tensor):
            assert scfg.top_p.dtype == torch.float32, (
                f"scfg.top_p.dtype ({scfg.top_p.dtype}) must be torch.float32"
            )
            assert scfg.top_p.shape[0] == batch_size, (
                f"scfg.top_p.shape[0] ({scfg.top_p.shape[0]}) must equal to batch_size ({batch_size})"
            )
        ...
        if isinstance(scfg.temperature, torch.Tensor):
            assert scfg.temperature.dtype == torch.float32, (
                f"scfg.temperature.dtype ({scfg.temperature.dtype}) must be torch.float32"
            )
            assert scfg.temperature.shape[0] == batch_size, (
                f"scfg.temperature.shape[0] ({scfg.temperature.shape[0]}) must equal to batch_size ({batch_size})"
            )
            self.temperature = scfg.temperature
        else:
            self.temperature = torch.full([batch_size], scfg.temperature, dtype=torch.float32)

        if isinstance(scfg.repetition_penalty, torch.Tensor):
            assert scfg.repetition_penalty.dtype == torch.float32, (
                f"scfg.repetition_penalty.dtype ({scfg.repetition_penalty.dtype}) must be torch.float32"
            )
            assert scfg.repetition_penalty.shape[0] == batch_size, (
                f"scfg.repetition_penalty.shape[0] ({scfg.repetition_penalty.shape[0]}) must equal to batch_size ({batch_size})"
            )
            ...
            self.repetition_penalty = torch.full(
                [batch_size], scfg.repetition_penalty, dtype=torch.float32
            )

        if isinstance(scfg.length_penalty, torch.Tensor):
            assert scfg.length_penalty.dtype == torch.float32, (
                f"scfg.length_penalty.dtype ({scfg.length_penalty.dtype}) must be torch.float32"
            )
            assert scfg.length_penalty.shape[0] == batch_size, (
                f"scfg.length_penalty.shape[0] ({scfg.length_penalty.shape[0]}) must equal to batch_size ({batch_size})"
            )
            self.host_length_penalty = scfg.length_penalty
        else:
            self.host_length_penalty = torch.full(
                [batch_size], scfg.length_penalty, dtype=torch.float32
            )
        self.length_penalty = self.host_length_penalty.to(self.device)

        if isinstance(scfg.early_stopping, torch.Tensor):
            assert scfg.early_stopping.dtype == torch.int32, (
                f"scfg.early_stopping.dtype ({scfg.early_stopping.dtype}) must be torch.int32"
            )
            assert scfg.early_stopping.shape[0] == batch_size, (
                f"scfg.early_stopping.shape[0] ({scfg.early_stopping.shape[0]}) must equal to batch_size ({batch_size})"
            )
            self.host_early_stopping = scfg.early_stopping
        else:
            self.host_early_stopping = torch.full(
                [batch_size], scfg.early_stopping, dtype=torch.int32
            )

        if isinstance(scfg.presence_penalty, torch.Tensor):
            assert scfg.presence_penalty.dtype == torch.float32, (
                f"scfg.presence_penalty.dtype ({scfg.presence_penalty.dtype}) must be torch.float32"
            )
            assert scfg.presence_penalty.shape[0] == batch_size, (
                f"scfg.presence_penalty.shape[0] ({scfg.presence_penalty.shape[0]}) must equal to batch_size ({batch_size})"
            )
            self.presence_penalty = scfg.presence_penalty
        elif scfg.presence_penalty == 0.0:
            self.presence_penalty = None
        else:
            self.presence_penalty = torch.full(
                [batch_size], scfg.presence_penalty, dtype=torch.float32
            )

        if isinstance(scfg.frequency_penalty, torch.Tensor):
            assert scfg.frequency_penalty.dtype == torch.float32, (
                f"scfg.frequency_penalty.dtype ({scfg.frequency_penalty.dtype}) must be torch.float32"
            )
            assert scfg.frequency_penalty.shape[0] == batch_size, (
                f"scfg.frequency_penalty.shape[0] ({scfg.frequency_penalty.shape[0]}) must equal to batch_size ({batch_size})"
            )
            self.frequency_penalty = scfg.frequency_penalty
        elif scfg.frequency_penalty == 0.0:
            self.frequency_penalty = None
        else:
            self.frequency_penalty = torch.full(
                [batch_size], scfg.frequency_penalty, dtype=torch.float32
            )

        if isinstance(scfg.min_length, torch.Tensor):
            assert scfg.min_length.dtype == torch.int32, (
                f"scfg.min_length.dtype ({scfg.min_length.dtype}) must be torch.int32"
            )
            assert scfg.min_length.shape[0] == batch_size, (
                f"scfg.min_length.shape[0] ({scfg.min_length.shape[0]}) must equal to batch_size ({batch_size})"
            )
            self.min_length = scfg.min_length
        else:
            self.min_length = torch.full([batch_size], scfg.min_length, dtype=torch.int32)

        if isinstance(scfg.beam_search_diversity_rate, torch.Tensor):
            assert scfg.beam_search_diversity_rate.dtype == torch.float32, (
                f"scfg.beam_search_diversity_rate.dtype ({scfg.beam_search_diversity_rate.dtype}) must be torch.float32"
            )
            assert scfg.beam_search_diversity_rate.shape[0] == batch_size, (
                f"scfg.beam_search_diversity_rate.shape[0] ({scfg.beam_search_diversity_rate.shape[0]}) must equal to batch_size ({batch_size})"
            )
            ...
            self.beam_search_diversity_rate = torch.full(
                [batch_size], scfg.beam_search_diversity_rate, dtype=torch.float32
            )
        else:
            self.beam_search_diversity_rate = None

        if isinstance(scfg.random_seed, torch.Tensor):
            assert scfg.random_seed.dtype == torch.int64, (
                f"scfg.random_seed.dtype ({scfg.random_seed.dtype}) must be torch.int64"
            )
            assert scfg.random_seed.shape[0] == batch_size, (
                f"scfg.random_seed.shape[0] ({scfg.random_seed.shape[0]}) must equal to batch_size ({batch_size})"
            )
            self.random_seed = scfg.random_seed
        elif scfg.random_seed is not None:
            self.random_seed = torch.full([batch_size], scfg.random_seed, dtype=torch.int64)
        else:
            self.random_seed = None

        if isinstance(scfg.no_repeat_ngram_size, torch.Tensor):
            assert scfg.no_repeat_ngram_size.dtype == torch.int32, (
                f"scfg.no_repeat_ngram_size.dtype ({scfg.no_repeat_ngram_size.dtype}) must be torch.int32"
            )
            assert scfg.no_repeat_ngram_size.shape[0] == batch_size, (
                f"scfg.no_repeat_ngram_size.shape[0] ({scfg.no_repeat_ngram_size.shape[0]}) must equal to batch_size ({batch_size})"
            )
            self.no_repeat_ngram_size = scfg.no_repeat_ngram_size
        elif scfg.no_repeat_ngram_size is not None:
            self.no_repeat_ngram_size = torch.full(
                [batch_size], scfg.no_repeat_ngram_size, dtype=torch.int32
            )
        else:
            self.no_repeat_ngram_size = None

        if isinstance(scfg.min_p, torch.Tensor):
            assert scfg.min_p.dtype == torch.float32, (
                f"scfg.min_p.dtype ({scfg.min_p.dtype}) must be torch.float32"
            )
            assert scfg.min_p.shape[0] == batch_size, (
                f"scfg.min_p.shape[0] ({scfg.min_p.shape[0]}) must equal to batch_size ({batch_size})"
            )
            self.min_p = scfg.min_p
        elif scfg.min_p == 1.0:
            self.min_p = None
        else:
            self.min_p = torch.full([batch_size], scfg.min_p, dtype=torch.float32)
        ...
        self.end_ids = torch.full((batch_size,), scfg.end_id, dtype=torch.int32, device=self.device)
        ...
        if input_ids.dim() == 1:
            ...
            split_ids_list = list(
                torch.split(input_ids.unsqueeze(0), host_context_lengths.numpy().tolist(), dim=1)
            )
            padded_input_ids = torch.nested.to_padded_tensor(
                torch.nested.nested_tensor(split_ids_list, dtype=torch.int32, device="cuda"),
                scfg.pad_id,
            ).reshape(batch_size, max_context_length)
        else:
            padded_input_ids = input_ids
        if scfg.num_beams > 1:
            tiled_input_ids = _tile_beam_width(padded_input_ids, scfg.num_beams)
            tiled_input_ids = tiled_input_ids.reshape(
                batch_size, scfg.num_beams, max_context_length
            )
            ...
            self.output_ids = torch.cat(
                (
                    tiled_input_ids,
                    torch.full(
                        (batch_size, scfg.num_beams, self.max_seq_length - max_context_length),
                        scfg.end_id,
        ...
        self.parent_ids = torch.zeros(
            (batch_size, scfg.num_beams, self.max_seq_length), dtype=torch.int32, device=self.device
        )
        ...
        if self.is_redrafter_mode:
            ...
            self.accept_lengths = torch.ones([batch_size], dtype=torch.int32, device=self.device)
            self.buffer["redrafter_inverted_temperature"] = torch.reciprocal(self.temperature).to(
                device=self.device, dtype=self.dtype
            )
        elif self.is_medusa_mode:
            self.new_tokens = torch.zeros(
                [batch_size, self.num_medusa_heads + 1], dtype=torch.int32, device=self.device
            )
            self.medusa_output_tokens = torch.zeros(
                [batch_size, self.num_draft_tokens], dtype=torch.int32, device=self.device
            )
            self.generation_input_ids = torch.zeros(
                [batch_size, self.num_draft_tokens + 1], dtype=torch.int32, device=self.device
            )
            self.accept_lengths = torch.ones([batch_size], dtype=torch.int32, device=self.device)
            ...
            self.new_tokens = torch.zeros(
                [batch_size, scfg.num_beams, 1], dtype=torch.int32, device=self.device
            )
        else:
            self.new_tokens = torch.zeros([batch_size, 1], dtype=torch.int32, device=self.device)

        if scfg.num_beams > 1 or scfg.output_cum_log_probs:
            self.cum_log_probs = torch.full(
                (batch_size, scfg.num_beams), -1e20, dtype=torch.float32, device=self.device
            )
        ...
        if scfg.output_log_probs:
            ...
            self.log_probs_tiled = torch.zeros(
                (self.max_seq_length, self._model_config.max_batch_size, scfg.num_beams),
                dtype=torch.float32,
                device=self.device,
            )
        else:
        ...
        self.finished = torch.zeros(
            (batch_size, scfg.num_beams), dtype=torch.uint8, device=self.device
        )
        ...
        if scfg.use_beam_hyps:
            ...
            self.beam_hyps_seq_len_cba = torch.zeros(
                [batch_size, scfg.num_beams * 2], dtype=torch.int32, device=self.device
            )
            self.beam_hyps_cum_log_probs_cba = torch.zeros(
                [batch_size, scfg.num_beams * 2], dtype=torch.float, device=self.device
            )
            ...
            self.beam_hyps_min_normed_scores = torch.zeros(
                [batch_size], dtype=torch.float, device=self.device
            )
            self.beam_hyps_num_beams = torch.zeros(
                [batch_size], dtype=torch.int32, device=self.device
            )
            self.beam_hyps_is_done = torch.zeros([batch_size], dtype=torch.bool, device=self.device)
        else:
    ...
    def _init_medusa(self, medusa_choices: List[List[int]]):
        from tensorrt_llm.runtime.medusa_utils import _medusa_setup, expand_choices_if_needed
        ...
        self.spec_decoding_packed_mask = (
            medusa_info.medusa_packed_mask.unsqueeze(0)
            .expand(target_shape)
            .reshape(-1, target_shape[-1])
            .cuda()
        )
        ...
        # Expand medusa position offsets to number of batch size in order to be compatible with the new Medusa.
        target_shape = list(medusa_info.medusa_position_offsets.unsqueeze(0).shape)
        target_shape[0] = self.batch_size
        ...
        self.spec_decoding_position_offsets = (
            medusa_info.medusa_position_offsets.unsqueeze(0).expand(target_shape).int().cuda()
        )
        # Fixed sequence lengths currently.
        # Support variable sequence lengths later.
        self.spec_decoding_generation_lengths = (
            (torch.ones((self.batch_size)) * (self.num_draft_tokens + 1)).int().cuda()
        )
        if not self.use_gpt_attention_plugin:
            medusa_fp_mask = torch.zeros_like(self.medusa_mask, dtype=torch.float32)
            medusa_fp_mask[torch.logical_not(self.medusa_mask)] = float("-inf")
            self.medusa_mask = medusa_fp_mask
        return

    def _get_num_paged_blocks(self, max_attention_window_size, sink_token_length):
        bubble_len = 0
        if sink_token_length % self.tokens_per_block > 0:
            bubble_len += self.tokens_per_block - sink_token_length % self.tokens_per_block
    ...
    def setup(
        ...
        if max_attention_window_size is None:
            ...
            self.host_max_attention_window_sizes = (
                torch.ones((self.num_attn_layers,), dtype=torch.int32)
            ...
            self.max_attention_window_size = min(max_attention_window_size, self.max_seq_length)
            self.host_max_attention_window_sizes = (
                torch.ones((self.num_attn_layers,), dtype=torch.int32)
                * self.max_attention_window_size
            )
            ...
            self.max_attention_window_size = int(torch.max(max_attention_window_size).item())
            ...
            if attn_win_size_len < num_total_attn_layers:
                ...
                warning_info = (
                    "The size of max_attention_window_size tensor/list is less than num_attn_layers, "
                ...
                warning_info += (
                    f" + {max_attention_window_size.tolist()[0:remain_num]}. "
                    if remain_num > 0
                    else ". "
                )
            ...
            self.max_attention_window_size = min(
                self.max_attention_window_size, self.max_seq_length
            )
            ...
            for i in range(self.num_attn_layers):
                self.host_max_attention_window_sizes[i] = max_attention_window_size[
                    (self.layer_types[0 : self.first_layer].count("attention") + i)
                    % attn_win_size_len
                ]
        else:
        ...
        if sink_token_length is None:
            ...
            self.host_sink_token_length = (
                torch.ones((1,), dtype=torch.int32) * self.sink_token_length
            )
        else:
        ...
        if self.mapping.is_last_pp_rank():
            if self.is_redrafter_mode:
                ...
                self.buffer["logits"] = torch.empty(
                    (batch_size, self.max_draft_tokens + 1, self.vocab_size_padded)
                ...
                self.buffer["logits"] = torch.empty(
                    (batch_size, self.num_draft_tokens + 1, self.vocab_size_padded)
                    if not self.gather_context_logits
                    else (batch_size, max_context_length, self.vocab_size_padded),
                    dtype=self._tensor_dtype("logits"),
                    device=self.device,
        ...
        if self.quant_mode.has_kv_cache_quant():
            ...
            if self.use_kv_cache and self.has_attn_layers:
                first_atten_layer = (
                    self.layer_types[self.first_layer : self.last_layer].index("attention")
                    + self.first_layer
                )
                kv_cache_type = (
                    self.dtype
                    if self.paged_kv_cache
                    else self._tensor_dtype(f"present_key_value_{first_atten_layer}")
                )
            else:
                kv_cache_type = None
        ...
        if self.use_kv_cache:
            if self.paged_kv_cache and self.has_attn_layers:
                ...
                self._memory_pool_allocator.allocate(kv_cache_type, num_kv_heads_per_layer)
                ...
                if self.cross_attention:  # As for now we enable cross paged kv and self paged kv to share the same tokens_per_block
                    ...
                    if self._model_config.num_kv_heads_per_cross_attn_layer is None:
                        num_kv_heads_per_cross_attn_layer = (
                            MemoryPoolsAllocator.prepare_num_kv_heads_per_layer(
                                self.get_num_heads_kv(), self.num_attn_layers
                            )
                        )
                    else:
                        num_kv_heads_per_cross_attn_layer = (
                            self._model_config.num_kv_heads_per_cross_attn_layer
                        )
        ...
        if self.use_gpt_attention_plugin:
            self.sequence_length_buffer = torch.ones(
                (batch_size,), dtype=torch.int32, device=self.device
            )
        else:
            ...
            if (
                self.use_kv_cache and self.has_attn_layers
        ...
        if self.use_gemm_allreduce_plugin:
            max_num_tokens = max(batch_size * beam_width, batch_size * self.max_seq_length)
            M = max_num_tokens
            N = self.hidden_size
        ...
        if self.is_medusa_mode:
            self.buffer["spec_decoding_packed_mask"] = self.spec_decoding_packed_mask
            self.buffer["spec_decoding_position_offsets"] = self.spec_decoding_position_offsets
            self.buffer["spec_decoding_generation_lengths"] = self.spec_decoding_generation_lengths
    ...
    def _allocate_empty_kv_cache_pools(self, kv_cache_type, num_blocks):
        ...
        if self._model_config.num_kv_heads_per_layer is None:
            ...
            unique_cache_pools.append(
                torch.empty(cache_shape, dtype=kv_cache_type, device=self.device)
            )

        # Layers are not homogeneous, use new kv cache shape
        else:
            kv_heads_unique_counter = Counter(self._model_config.num_kv_heads_per_layer)
    ...
    def _get_context_shape_buffer(
        ...
        def add_tensor_from_pointer(pointer, name, shape, str_dtype):
            return tensors.update(
                {name: RuntimeTensor.from_pointer(name, pointer, shape, str_dtype)}
            )
        ...
        def add_tensor_with_shape(x, name, shape):
            return tensors.update({name: RuntimeTensor.from_torch(name, x, override_shape=shape)})
        ...
        if self.has_attn_layers:
            if self.use_gpt_attention_plugin:
                ...
                assert host_runtime_perf_knobs != None, (
                    "gpt_attention_plugin needs to set host_runtime_perf_knobs"
                )
        ...
        if self.cross_attention:
            ...
            add_tensor(torch.ones(1, dtype=torch.bool, device=self.device), "cross_kv_cache_gen")
            ...
            if self.skip_cross_kv:
                if self.cross_kv_reuse is None:
                    ...
                    cross_kv_out_dim = 2 * self.get_num_heads_kv() * self.head_size
                    cross_kv_shape = encoder_output.shape[:-1] + (cross_kv_out_dim,)
                    cross_kv_reuse = torch.empty(
                        cross_kv_shape, dtype=encoder_output.dtype, device=encoder_output.device
                    )
            ...
            if language_adapter_routings is not None:
                add_tensor(language_adapter_routings, "language_adapter_routings")
            ...
            if not self.use_gpt_attention_plugin:
                ...
                if cross_attention_mask != None:
                    ...
                    cross_attention_packed_mask = torch.ops.tensorrt_llm.pack_fmha_mask_by_input(
                        cross_attention_mask, context_lengths, encoder_input_lengths, 1.0
                    )
                    add_tensor(cross_attention_mask, "cross_attention_mask")
                    add_tensor(cross_attention_packed_mask, "cross_attention_packed_mask")
                else:
        ...
        if self.mapping.has_pp():
            ...
            if input_ids.dim() == 2:
                ...
                hidden_states_input = hidden_states_input.resize_(input_ids.shape[0], hidden_size)
        ...
        if self.paged_kv_cache and self.has_attn_layers:
            ...
            add_tensor_with_shape(
                host_kv_cache_block_offsets, f"host_kv_cache_block_offsets", shape
            )
            ...
            if self.cross_attention:
                ...
                cross_shape = [cross_shape[0], cross_shape[1] * cross_shape[2], *cross_shape[3:]]
                add_tensor_with_shape(cross_buffer, f"cross_kv_cache_block_offsets", cross_shape)
                add_tensor_with_shape(
                ...
                add_tensor(self.buffer[cross_pool_pointers], cross_pool_pointers)
                add_tensor(self.buffer[cross_pool_mapping], cross_pool_mapping)

        batch_size = context_lengths.shape[0]
        if self.use_kv_cache and not self.paged_kv_cache:
            for idx in range(self.first_layer, self.last_layer):
                if not self.use_gpt_attention_plugin and self.layer_types[idx] == "attention":
                    kv_cache_shape = (
                        batch_size,
                        2,
                        self.get_num_heads_kv(self.general_to_attn_idx[idx]),
                        0,
                        self.head_size,
                    )
                    ...
                    if self.cross_attention:
                        ...
                        # for empty tensor, TRT does not really use the tensor data, so any dtype is fine
                        cross_kv_cache_buffer = torch.zeros(
        ...
        for idx in range(self.first_layer, self.last_layer):
            if self.layer_types[idx] != "recurrent":
                continue
            if self.paged_state:
                add_tensor(self.buffer[f"conv_state_ptr_{idx}"], f"conv_state_ptr_{idx}")
                add_tensor(self.buffer[f"rnn_state_ptr_{idx}"], f"rnn_state_ptr_{idx}")
            else:
                # conv state
                dtype = self._tensor_dtype(f"present_conv_state_{idx}")
                if self.use_mamba_conv1d_plugin:
                    conv_state_shape = (batch_size, self.conv_kernel - 1, self.rnn_conv_dim_size)
                else:
                    conv_state_shape = (batch_size, self.rnn_conv_dim_size, self.conv_kernel - 1)

                conv_state = torch.zeros(conv_state_shape, dtype=dtype, device=self.device)
        ...
        if self.paged_state and self.has_rnn_layers:
            slot_mapping = torch.arange(0, batch_size, device="cuda", dtype=torch.int32)
        ...
        if self.use_gpt_attention_plugin and self.has_attn_layers:
            ...
            if self.is_redrafter_mode:
                device_request_types = torch.zeros_like(context_lengths, device=self.device).int()
            ...
            add_tensor_with_shape(
                host_context_lengths.clone(), "host_past_key_value_lengths", (batch_size,)
            )
            add_tensor_with_shape(self.host_sink_token_length, "host_sink_token_length", (1,))
        ...
        if self.use_lora_plugin:
            ...
            if self.cross_attention and self.remove_input_padding:
                add_tensor(encoder_input_lengths.to("cpu"), "host_encoder_input_lengths")
        if self.is_medusa_mode:
            # Medusa mask and position offsets are fixed for the whole session.
            add_tensor(self.buffer["spec_decoding_packed_mask"], "spec_decoding_packed_mask")
            add_tensor(
                self.buffer["spec_decoding_position_offsets"], "spec_decoding_position_offsets"
            )
            add_tensor(
                self.buffer["spec_decoding_generation_lengths"], "spec_decoding_generation_lengths"
            )
    ...
    def _get_next_step_shape_buffer(
        ...
        if self.mapping.is_first_pp_rank():
            if self.is_redrafter_mode:
                ...
                input_ids_shape = (
                    ...
                    if self.remove_input_padding
                    else (batch_size * beam_width, self.num_draft_tokens + 1)
                )
            if self.is_redrafter_mode:
                add_tensor_with_shape(self.buffer["flat_tokens"], "input_ids", input_ids_shape)
            elif self.is_medusa_mode:
                add_tensor_with_shape(self.generation_input_ids, "input_ids", input_ids_shape)
            else:
        ...
        if self.cross_attention:
            ...
            add_tensor(torch.zeros(1, dtype=torch.bool, device=self.device), "cross_kv_cache_gen")
            if self._model_config.skip_cross_attn_blocks:
                add_tensor(skip_cross_attn_blocks, "skip_cross_attn_blocks")
            add_tensor_with_shape(encoder_output, "encoder_output", encoder_output_shape)
            add_tensor(encoder_input_lengths, "encoder_input_lengths")
            if language_adapter_routings is not None:
                add_tensor(language_adapter_routings, "language_adapter_routings")
            add_tensor(self.buffer["encoder_max_input_length"], "encoder_max_input_length")
            if not self.use_gpt_attention_plugin:
                ...
                if cross_attention_mask != None:
                    cross_attention_mask = _tile_beam_width(cross_attention_mask, beam_width)
                    # Empty packed mask is passed in the generation phase as it is not used.
                    cross_attention_packed_mask = torch.empty(
                        (batch_size, (cross_attention_mask.shape[1] + 31) // 32),
                        dtype=torch.int32,
                        device=self.device,
                    )
                    add_tensor(cross_attention_mask, "cross_attention_mask")
                    ...
                    add_tensor(
                        torch.ones(
                            (batch_size, np.asarray(list(encoder_output.shape)[:-1]).prod()),
                            dtype=torch.bool,
                            device=self.device,
                        ),
                        "cross_attention_mask",
                    ...
                    # Empty packed mask is passed in the generation phase as it is not used.
                    add_tensor(
                        torch.empty((batch_size, 1), dtype=torch.int32, device=self.device),
        ...
        if self.paged_kv_cache and self.has_attn_layers:
            shape = kv_cache_block_offsets.shape
            shape = [shape[0], shape[1] * shape[2], *shape[3:]]
            add_tensor_with_shape(kv_cache_block_offsets, f"kv_cache_block_offsets", shape)
            add_tensor_with_shape(
                host_kv_cache_block_offsets, f"host_kv_cache_block_offsets", shape
            )
            pool_pointers = f"host_kv_cache_pool_pointers"
            ...
            if self.cross_attention:
                ...
                cross_shape = [cross_shape[0], cross_shape[1] * cross_shape[2], *cross_shape[3:]]
                add_tensor_with_shape(
                    cross_kv_cache_block_offsets, f"cross_kv_cache_block_offsets", cross_shape
                )
                add_tensor_with_shape(
                    host_cross_kv_cache_block_offsets,
                    f"host_cross_kv_cache_block_offsets",
        ...
        if not self.paged_kv_cache:
            for attn_idx, layer_idx in self.attn_to_general_idx.items():
                if not self.use_gpt_attention_plugin:
                    ...
                    add_tensor_with_shape(
                        self.buffer[input_name], f"past_key_value_{layer_idx}", next_shape
                    )
                    add_tensor(self.buffer[output_name], f"present_key_value_{layer_idx}")
                else:
                    key_value_cache = self.buffer[f"present_key_value_{layer_idx}"]
                    add_tensor(key_value_cache, f"past_key_value_{layer_idx}")
                    add_tensor(key_value_cache, f"present_key_value_{layer_idx}")

                    if self.cross_attention:
                        cross_cache_buffer = self.buffer[f"cross_present_key_value_{layer_idx}"]
                        add_tensor(cross_cache_buffer, f"cross_past_key_value_{layer_idx}")
                        add_tensor(cross_cache_buffer, f"cross_present_key_value_{layer_idx}")
        ...
        for idx in range(self.first_layer, self.last_layer):
            ...
            if self.paged_state:
                ...
                if step % 2:
                    ...
                    add_tensor(
                        self.buffer[f"present_conv_state_{idx}"], f"present_conv_state_{idx}"
                    )
                else:
                    add_tensor_with_shape(
                    ...
                    add_tensor(
                        self.buffer[f"1_present_conv_state_{idx}"], f"present_conv_state_{idx}"
                    )
                # rnn state
                rnn_state = self.buffer[f"present_rnn_state_{idx}"]
                add_tensor(rnn_state, f"past_rnn_state_{idx}")
        ...
        if self.use_gpt_attention_plugin and self.has_attn_layers:
            ...
            if self.is_redrafter_mode:
                ...
                device_request_types = torch.ones_like(context_lengths, device=self.device).int()
                add_tensor(device_request_types, "device_request_types")
            ...
            add_tensor_with_shape(sequence_length, "sequence_length", (batch_size * beam_width,))
            add_tensor_with_shape(self.host_sink_token_length, "host_sink_token_length", (1,))
            add_tensor_with_shape(
                self.host_max_attention_window_sizes,
                f"host_max_attention_window_sizes",
        ...
        if self.is_redrafter_mode:
            set_redrafter_gen_tensors(self, batch_size, add_tensor, add_tensor_with_shape)
        torch.cuda.nvtx.range_pop()
    ...
    def _prepare_context_inputs(
        ...
        **kwargs,
        ...
        if (self.is_medusa_mode or self.is_redrafter_mode) and not remove_input_padding:
        ...
        if (use_gpt_attention_plugin or self.has_rnn_layers) and remove_input_padding:
        ...
        if use_gpt_attention_plugin:
            ...
            if remove_input_padding:
                position_ids = torch.concat(
                    [
                        torch.arange(0, host_context_lengths[i], dtype=torch.int32, device="cuda")
                        for i in range(batch_size)
                    ]
                )
            else:
                position_ids = (
                    torch.tensor(range(max_context_length), dtype=torch.int32, device="cuda")
                    .reshape([1, -1])
                    .expand([batch_size, -1])
                )

            perf_knob_tensor_size = 16
            context_runtime_perf_knobs = torch.tensor(
                [-1] * perf_knob_tensor_size, dtype=torch.int64
            )
    ...
    def _prepare_generation_inputs(
        self, batch_size, context_lengths, use_gpt_attention_plugin, remove_input_padding, **kwargs
    ):
        torch.cuda.nvtx.range_push("_prepare_generation_inputs")

        step = kwargs.pop("step")
        last_token_ids = torch.ones_like(context_lengths)
        if use_gpt_attention_plugin and (self.is_medusa_mode or self.is_redrafter_mode):
            if remove_input_padding:
                if self.is_medusa_mode:
                    ...
                    last_token_ids = torch.ones(
                        self.host_total_gen_token, dtype=torch.int32, device=context_lengths.device
                    )
                    torch.cuda.nvtx.range_pop()
            else:
                # For Medusa, last_token_ids should be [bs, seq] and should contain the actual indices (starts from 0)
                last_token_ids = torch.arange(
                    self.num_draft_tokens + 1, dtype=torch.int32, device=context_lengths.device
                )
        ...
        if use_gpt_attention_plugin:
            if self.is_redrafter_mode:
                ...
                if step != 0:
                    self.buffer["position_ids_base"] += self.buffer["num_accepted_tokens"]
                position_ids = self.buffer["packed_position_ids"].view(-1)[
                    : self.host_total_gen_token
                ]
                if step == 0:
                    position_ids -= 1
            ...
            perf_knob_tensor_size = 16
            gen_runtime_perf_knobs = torch.tensor([-1] * perf_knob_tensor_size, dtype=torch.int64)
            if self.multi_block_mode:
            ...
            attention_mask = torch.cat(
                (attention_mask, attention_mask.new_ones((batch_size * num_beams, 1))), dim=-1
            ).contiguous()
            position_ids = attention_mask.long().cumsum(-1) - 1
            position_ids.masked_fill_(attention_mask == 0, 1)
    ...
    def _prepare_cross_attention_mask(self, batch_size, context_lengths, cross_attention_mask):
        ...
        for batch_idx in range(batch_size):
            ...
            local_mask_for_context = cross_attention_mask[batch_idx][:decoder_input_length, :]
            local_mask_for_gen = cross_attention_mask[batch_idx][decoder_input_length:, :]
            if not self.use_gpt_attention_plugin:
            ...
            if not self.remove_input_padding:
                local_mask_for_context = torch.nn.functional.pad(
                    ...
                    (0, 0, 0, (max_decoder_input_length - decoder_input_length)),
                    "constant",
                    False,
                )
    ...
    def pp_communicate_new_tokens(self, should_stop, cache_indir, sequence_length):
        if self.mapping.is_last_pp_rank():
    ...
    def pp_communicate_final_output_ids(self, final_output_ids, batch_size, beam_width):
        if self.mapping.is_last_pp_rank():
            ...
            final_output_ids = torch.zeros(
                (batch_size, beam_width, self.max_seq_length), dtype=torch.int32, device=self.device
            )
            self.nccl_comm.recv(final_output_ids, self.mapping.pp_group[-1])
        return final_output_ids

    def finalize_decoder(self, context_lengths, batch_size, beam_width, scfg, in_progress=False):
    ...
    def find_best_medusa_path(self, batch_size, input_ids: torch.Tensor, next_logits, temp=0):
        ...
        zero_pad = torch.zeros((batch_size, 1), dtype=input_ids.dtype, device=input_ids.device)
        ...
        if temp == 0:
            ...
            input_paths = [input_ids[b, self.medusa_paths] for b in range(batch_size)]
            new_paths = [new_tokens[b, self.medusa_paths] for b in range(batch_size)]
            for b in range(batch_size):
                equality = input_paths[b][:, 1:] == new_paths[b][:, :-1]
                paths_correct_len = torch.cumprod(equality.int(), dim=1).sum(dim=1)
                best_path_len[b] = paths_correct_len.max().item() + 1
                if best_path_len[b] > 1:
                    best_path[b] = torch.argmax(paths_correct_len)
                next_tokens[b] = new_paths[b][best_path[b]][: best_path_len[b]].clone()

        return best_path, best_path_len, next_tokens

    def filter_medusa_logits(self, batch_size, best_path, best_path_lengths, medusa_logits):
        """
        ...
        medusa_logits = medusa_logits.view(
            self.num_medusa_heads, batch_size, self.num_draft_tokens + 1, -1
        )
        for b in range(batch_size):
    ...
    def get_next_medusa_tokens(self, batch_size, next_medusa_logits):
        next_medusa_tokens = [
            torch.zeros((batch_size, 1), dtype=torch.int32, device=next_medusa_logits.device)
        ]  # dummy token for now, TODO: update tree_ids and remove this
        for i in range(self.num_medusa_heads):
            medusa_token = torch.topk(
                next_medusa_logits[i, :, :], self.medusa_topks[i], dim=-1
            ).indices
            next_medusa_tokens.append(medusa_token)
        next_medusa_tokens = torch.cat(next_medusa_tokens, dim=-1)
        return next_medusa_tokens

    def locate_accepted_draft_tokens(self, batch_size, best_path, best_path_len, draft_paths):
        torch.cuda.nvtx.range_push("locate_accepted_draft_tokens")
        best_path_len_tensor = (
            best_path_len
            if isinstance(best_path_len, torch.Tensor)
            else torch.tensor(best_path_len, dtype=torch.int, device="cuda")
        )
        accepted_draft_token_counts = torch.maximum(
            best_path_len_tensor - 1, torch.tensor([0], device=best_path_len_tensor.device)
        )
        accepted_draft_token_offsets = torch.zeros(batch_size + 1, dtype=torch.int32, device="cuda")
        accepted_draft_token_offsets[1:] = torch.cumsum(accepted_draft_token_counts, dim=0)
        accepted_draft_token_offsets_cpu = accepted_draft_token_offsets.to("cpu")
        packed_accepted_draft_tokens_indices = torch.empty(
            accepted_draft_token_offsets_cpu[batch_size], dtype=torch.int32, device="cuda"
        )
        for seq_idx in range(batch_size):
            cur_draft_paths = draft_paths if self.is_medusa_mode else draft_paths[seq_idx]
            ...
            best_path_idx = (
                best_path[seq_idx].cpu()
                if isinstance(best_path[seq_idx], torch.Tensor)
                else best_path[seq_idx]
            )
            seq_accepted_token_indices = cur_draft_paths[
                best_path_idx, 1 : 1 + seq_accepted_draft_count
            ]
            packed_accepted_draft_tokens_indices[seq_start:seq_end] = seq_accepted_token_indices - 1
    ...
    def update_output_ids_by_offset(self, new_generated_ids, offsets):
        ...
        for b in range(batch_size):
            self.output_ids[b, offsets[b] : (offsets[b] + self.accept_lengths[b])] = (
    ...
    def next_medusa_input_ids(self):
        ...
        for b in range(batch_size):
            self.generation_input_ids[b, 0] = self.new_tokens[b, self.accept_lengths[b] - 1]
    ...
    def reorder_kv_cache_for_beam_search(
        ...
        for i in self.attn_to_general_idx:
            ...
            parent_ids = self.parent_ids[..., max_context_length + step].view(-1)
    ...
    def early_stop_criteria(self, batch_size, step, should_stop):
        for b in range(batch_size):
            ...
            # end id criteria.
            end_id_mask = self.new_tokens[b, : self.accept_lengths[b]] == self.end_ids[b]
            should_stop_with_end_id = torch.any(end_id_mask)
            self.medusa_should_stop[b] = (
                self.medusa_should_stop[b]
                or (prev_total_output_length + self.accept_lengths[b] >= self.max_new_tokens)
                or should_stop_with_end_id
            )
            # update accept lengths for the current step.
            if prev_total_output_length + self.accept_lengths[b] >= self.max_new_tokens:
                self.accept_lengths[b] = min(
                    self.max_new_tokens - prev_total_output_length, self.accept_lengths[b]
                )
            if should_stop_with_end_id:
                # get the position of first end_id.
                end_id_pos = (end_id_mask).nonzero(as_tuple=True)[0]
                self.accept_lengths[b] = min(end_id_pos[0] + 1, self.accept_lengths[b])
            self.total_accept_lengths[b] += self.accept_lengths[b]

        should_stop[0] = (
            should_stop[0]
            or (step == self.max_new_tokens - 1)
            or torch.all(self.medusa_should_stop)
        )
        return should_stop
    ...
    def medusa_decode_and_verify(self, step, batch_size, logits):
        ...
        if step == 0:
            ...
            logits = logits.view(-1)[: batch_size * logits.shape[-1]].view(batch_size, -1)
            next_main_token_logits = logits.to(self.decoder_logits_dtype)
            next_main_token = torch.argmax(next_main_token_logits, dim=-1, keepdim=True)
            self.new_tokens = next_main_token
            # NOTE: only one token's medusa logit will be written in.
            medusa_logits = medusa_logits.view(self.num_draft_tokens + 1, -1)[0, ...]
            next_medusa_logits = medusa_logits.reshape(self.num_medusa_heads, batch_size, -1).to(
                self.decoder_logits_dtype
            )
            next_medusa_tokens = self.get_next_medusa_tokens(batch_size, next_medusa_logits)
            self.medusa_output_tokens = next_medusa_tokens[
                :, self.medusa_tree_ids[-self.num_draft_tokens :]
            ]
            self.accept_lengths = torch.ones([batch_size], dtype=torch.int32, device=self.device)
        else:
            ...
            best_path, best_path_lengths, next_main_tokens = self.find_best_medusa_path(
                batch_size,
                self.generation_input_ids.view(batch_size, -1),
                next_token_logits.view(batch_size, self.num_draft_tokens + 1, -1),
            )
            self.accept_lengths = torch.tensor(best_path_lengths, device=self.device)
            self.new_tokens = torch.nested.to_padded_tensor(
                torch.nested.nested_tensor(next_main_tokens, dtype=torch.int32), self.end_ids[0]
            )  # FIXME  end id padding.
    ...
    def process_logits_including_draft(self, step, batch_size, logits, next_step_buffer):
        """
        ...
        if self.is_medusa_mode:
            ...
            best_path, best_path_lengths = self.medusa_decode_and_verify(step, batch_size, logits)
            ...
            last_draft_tokens_len = (
                self.buffer["next_spec_decoding_generation_lengths"] - 1 if step > 0 else 0
            )
            cur_draft_tokens_len = self.buffer["spec_decoding_generation_lengths"] - 1
        ...
        if step == 0:
            ...
            self.medusa_should_stop = torch.eq(self.new_tokens.reshape(-1), self.end_ids)
            should_stop[0] = torch.equal(self.new_tokens.reshape(-1), self.end_ids) or (
                step == self.max_new_tokens - 1
            )
        else:
            should_stop = self.early_stop_criteria(batch_size, step, should_stop)
        torch.cuda.nvtx.range_pop()
        ...
        self.update_output_ids_by_offset(
            self.new_tokens, self.sequence_length_buffer - last_draft_tokens_len
        )
        torch.cuda.nvtx.range_pop()
        ...
        if step != self.max_new_tokens - 1 and not should_stop.item():
            ...
            if step != 0:
                ...
                accepted_draft_token_offsets, packed_accepted_draft_tokens_indices = (
                    self.locate_accepted_draft_tokens(
                ...
                self.sequence_length_buffer += (
                    self.accept_lengths + cur_draft_tokens_len - last_draft_tokens_len
                )
            else:
    ...
    def handle_per_step(
        ...
        **kwargs,
        ...
        language_adapter_routings = kwargs.get("language_adapter_routings", None)
        if step == 0:
            ...
            context_runtime_perf_knobs = model_inputs.get("host_runtime_perf_knobs", None)
            ...
            if self.paged_kv_cache and self.has_attn_layers:
                ...
                if self.cross_attention:
                    host_cross_kv_cache_block_offsets = (
                        self.cross_pools_kv_cache_manager.get_block_offsets(beam_width=1)
                    )
                    cross_kv_cache_block_offsets = host_cross_kv_cache_block_offsets.to("cuda")
        ...
        stream = torch.cuda.current_stream().cuda_stream
        instance_idx = step % 2
        if self.cuda_graph_mode and self.runtime.cuda_graph_instances[instance_idx] is not None:
            # launch cuda graph
            CUASSERT(
                cudart.cudaGraphLaunch(self.runtime.cuda_graph_instances[instance_idx], stream)
            )
            ok = True
        else:
            ...
            if step == 0 and self.gather_context_logits:
                ...
                if self.remove_input_padding:
                    ...
                    self.buffer["logits"] = torch.gather(
                        self.buffer["logits"], dim=1, index=last_token_ids.to(dtype=torch.int64)
                    ).view(batch_size, self.vocab_size_padded)
        ...
        if step == 0 and beam_width > 1:
            ...
            if encoder_input_lengths is not None:
                encoder_input_lengths = _tile_beam_width(encoder_input_lengths, beam_width)
            ...
            if not self.paged_kv_cache:
                for key in self.buffer:
                    ...
                    if "present_key_value" in key:
                        if self.use_gpt_attention_plugin:
                            self.buffer[key] = _tile_beam_width(self.buffer[key], beam_width)
                        else:
            ...
            if self.mapping.is_last_pp_rank():
                self.buffer["logits"] = _tile_beam_width(self.buffer["logits"], beam_width)
        ...
        if (
            step == 0 and not self.is_medusa_mode and not self.is_redrafter_mode
        ):  # Medusa/ReDrafter has its own logic
            self.sequence_length_buffer = context_lengths.detach().clone()
        ...
        if not step == self.max_new_tokens - 1:
            ...
            if position_ids_raw is None:
                ...
                position_ids = torch.cat([p[-1:] + step + 1 for p in position_ids_raw], dim=0)
                if not self.remove_input_padding:
                    position_ids = torch.unsqueeze(position_ids, 1)
            last_token_ids = model_inputs.get("last_token_ids")
            attention_mask = model_inputs.get("attention_mask", None)
            gen_runtime_perf_knobs = model_inputs.get("host_runtime_perf_knobs", None)
            host_context_progress = torch.tensor([0], dtype=torch.int64)
            ...
            if self.paged_kv_cache and self.has_attn_layers:
                ...
                if self.cross_attention:
                    host_cross_kv_cache_block_offsets = (
                        self.cross_pools_kv_cache_manager.get_block_offsets(beam_width)
                    )
                    cross_kv_cache_block_offsets = host_cross_kv_cache_block_offsets.to("cuda")
                torch.cuda.nvtx.range_pop()
                ...
                if self.is_medusa_mode:
                    ...
                    if logits_processor is not None:
                        final_output_ids = self.finalize_decoder(
                            context_lengths, batch_size, beam_width, scfg, in_progress=True
                        )
                    ...
                    if stopping_criteria is not None and not should_stop.item():
                        ...
                        # keep the shape as same as huggingface stopping_criteria
                        final_output_ids_ = final_output_ids.reshape(-1, final_output_ids.size(-1))
                        should_stop[0] = stopping_criteria(step, final_output_ids_, logits)
        ...
        if self.debug_mode:
            ...
            if next_step_tensors is not None:
                self.debug_buffer = {
                    name: tensor.to_torch() for name, tensor in next_step_tensors.items()
                }
        ...
        **kwargs,
        ...
        def get_outputs_dict(output_ids, num_steps=self.max_new_tokens):
            ...
            if output_sequence_lengths:
                outputs["sequence_lengths"] = self.sequence_length_buffer.reshape(
                    [batch_size, beam_width]
                )
        ...
        def profile_fn(benchmark_profiler_obj, step_count):
            if benchmark_profiler_obj is not None:
                ...
                benchmark_profiler_obj.add_aux_info("generation_step_count", step_count)
        ...
        if cross_attention_mask is not None:
            ...
            if self.use_gpt_attention_plugin:
                ...
                cross_attention_mask_for_context = cross_attention_mask_for_context.to(torch.bool)
                cross_attention_mask_for_gen = cross_attention_mask_for_gen.to(torch.bool)
        ...
        assert not self.is_medusa_mode and not self.is_redrafter_mode, (
            "the custom decoder doesn't support medusa/redrafter."
        )
    ...
    def decode_batch(
        ...
        **kwargs,
        ...
        return self.decode(
            input_ids, context_lengths, sampling_config, streaming=streaming, **kwargs
        )
        ...
        **kwargs,
        ...
        if self.remove_input_padding and input_ids.dim() == 2:
            assert input_ids.shape[0] == 1, (
                "Packed 2D input must have shape [1, <sum of input lengths>]"
            )
        ...
        sequence_limit_lengths = torch.full(
            (batch_size, 1), self.max_seq_length, dtype=torch.int32, device=self.device
        )

        # Sequence_lengths for the dynamic decoder still has the input paddings.
        sequence_lengths = torch.full(
            (batch_size * beam_width, 1), max_context_length, dtype=torch.int32, device=self.device
        )
        ...
        if self.paged_kv_cache and self.has_attn_layers:
            ...
            self.buffer[f"host_kv_cache_pool_mapping"] = self._memory_pool_allocator.pool_mapping
            ...
            if self.cross_attention:
                ...
                self.buffer[f"host_cross_kv_cache_pool_pointers"] = (
                    self._cross_memory_pool_allocator.get_kv_cache_pool_pointers()
                )
                self.buffer[f"host_cross_kv_cache_pool_mapping"] = (
                    self._cross_memory_pool_allocator.pool_mapping
                )
            ...
            for bi in range(batch_size):
                ...
                self.pools_kv_cache_manager.add_sequence(generation_sequence, max_context_length)
                if self.cross_attention:
                    cross_generation_sequence = GenerationSequence(seq_idx=bi, batch_idx=bi)
        ...
        if self.is_medusa_mode or self.is_redrafter_mode:
            if self.quant_mode.has_kv_cache_quant():
                ...
                kv_cache_type = (
                    self.dtype
                    if self.paged_kv_cache
                    else self._tensor_dtype(f"present_key_value_{self.first_layer}")
                )
        ...
        if stop_words_list is not None:
            ...
            stop_words_lens = torch.full((batch_size,), max_stop_words_len, dtype=torch.int32).to(
                "cuda"
            )
            stop_words_list_ptrs = torch.zeros((batch_size), dtype=torch.int64)
            for bi in range(batch_size):
                stop_words_list_ptrs[bi] = (
                    stop_words_list.data_ptr()
                    + bi * 2 * max_stop_words_len * stop_words_list.element_size()
                )
            stop_words_list_ptrs = stop_words_list_ptrs.to("cuda")
        stop_words_data = (stop_words_list_ptrs, stop_words_lens, max_stop_words_len)
        ...
        if bad_words_list is not None:
            ...
            bad_words_lens = torch.full((batch_size,), max_bad_words_len, dtype=torch.int32).to(
                "cuda"
            )
            bad_words_list_ptrs = torch.zeros((batch_size), dtype=torch.int64)
            for bi in range(batch_size):
                bad_words_list_ptrs[bi] = (
                    bad_words_list.data_ptr()
                    + bi * 2 * max_bad_words_len * bad_words_list.element_size()
                )
            bad_words_list_ptrs = bad_words_list_ptrs.to("cuda")
        bad_words_data = (bad_words_list_ptrs, bad_words_lens, max_bad_words_len)
...
class ChatGLMGenerationSession(GenerationSession):
    ...
    def _prepare_context_inputs(
        ...
        if remove_input_padding:
            input_lengths_acc = torch.cumsum(
                torch.cat([torch.IntTensor([0]).cuda(), context_lengths], dim=0), dim=0
            )
            position_ids = torch.zeros([2, input_lengths_acc[-1]], dtype=torch.int32)
            for i in range(batch_size):
                position_ids[0, input_lengths_acc[i] : input_lengths_acc[i + 1]] = torch.arange(
                    0, context_lengths[i], dtype=torch.int32
                )
                position_ids[0, input_lengths_acc[i + 1] - 1] = context_lengths[i] - 2
            ...
            if kwargs["pad_id"] in [50256, 50259]:
                ...
                self.mask_index_tensor = torch.zeros([batch_size], dtype=torch.int32)
                position_ids = position_ids.cpu()
                for i in range(batch_size):
                    length = context_lengths[i]
                    input_ids = (
                        kwargs["input_ids"][0 : context_lengths[i]]
                        if i == 0
                        else kwargs["input_ids"][
                            sum(context_lengths[0:i]) : sum(context_lengths[0:i]) + length
                        ]
                    )
                    mask_index = [torch.where(input_ids == id)[0].int() for id in mask_ids]
                    ...
                    position_ids[0][sum(context_lengths[0 : i + 1]) - 1] = int(mask_index)
                position_ids = position_ids.cuda()
        else:
            position_ids = torch.zeros([batch_size, 2, max_context_length], dtype=torch.int32)
        ...
        perf_knob_tensor_size = 16
        context_runtime_perf_knobs = torch.tensor([-1] * perf_knob_tensor_size, dtype=torch.int64)
    ...
    def _prepare_generation_inputs(
        ...
        perf_knob_tensor_size = 16
        generation_runtime_perf_knobs = torch.tensor(
            [-1] * perf_knob_tensor_size, dtype=torch.int64
        )
...
class QWenForCausalLMGenerationSession(GenerationSession):
    ...
    def generate(
        ...
        max_new_tokens = min(max_new_tokens, self.global_max_output_length - max_input_length)
