...
<<<<<<< HEAD
    ...
    parser.add_argument(
        ...
        type=int,
        default=5184,
        help="Maximum multiply of h and w after patching for input images for qwen2_vl",
    )
    parser.add_argument(
        "--min_hw_dims",
        type=int,
        default=128,
        help="Minimum multiply of h and w after patching for input images for qwen2_vl",
    )
    parser.add_argument(
        "--num_mul_bins",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=3000,
        help="Maximum Mel frequency feature lengths of input audios for qwen2_audio",
    )
=======
        '--model_type',
        type=str,
...
<<<<<<< HEAD
        ...
        assert num_mul_bins > 0, "num_mul_bins must be positive for higgs_audio"
        assert max_mel_seq_len > 0, "max_mel_seq_len must be positive for higgs_audio"
...
<<<<<<< HEAD
        ...
        elif isinstance(input_sizes[0], list) and isinstance(input_sizes[0][0], list):
=======
...
<<<<<<< HEAD
    ...
    blip2_llm = ""
    if model.language_model.config.architectures[0] == "T5ForConditionalGeneration":
        blip2_llm = "t5"
...
def build_interlm_xcomposer2_engine(args):
    model = AutoModel.from_pretrained(args.model_path, trust_remote_code=True).to(torch.float16)
    raw_image = Image.new("RGB", [10, 10])
    image = model.vis_processor(raw_image).unsqueeze(0).to(args.device, torch.float16)
...
<<<<<<< HEAD
    ...
    model = Pix2StructForConditionalGeneration.from_pretrained(args.model_path, torch_dtype=dtype)
=======
            attention_mask = (image.abs().sum(dim=-1) != 0)
            vision_x = self.encoder.embeddings(image)
...
<<<<<<< HEAD
        ...
        _, model, processor, _ = load_pretrained_model(
            args.model_path, None, args.model_type, torch_dtype="float16"
        )
        raw_image = Image.new("RGB", [512, 512])
        image = (
            process_images([raw_image], processor, model.config)
            .squeeze(0)
...
<<<<<<< HEAD
        wrapper = LlavaQwenVisionWrapper(
            model.get_model().get_vision_tower(), model.get_model().mm_projector
        )
...
<<<<<<< HEAD
    ...
    if args.model_type == "llava_next":
        ...
        save_file(tensor_img_newline, os.path.join(args.output_dir, "image_newlines.safetensors"))
...
<<<<<<< HEAD
    ...
    wrapper = VilaVisionWrapper(
        model.get_vision_tower().to(args.device), model.mm_projector.to(args.device)
    )
    export_onnx(wrapper, image, f"{args.output_dir}/onnx")
    build_trt_engine(
        args.model_type,
...
<<<<<<< HEAD
    raw_image = Image.new("RGB", [10, 10])
    image = (
        processor(text="dummy", images=raw_image, return_tensors="pt")["image_patches"][0]
        .to(args.device, torch.float16)
        .unsqueeze(0)
    )
...
<<<<<<< HEAD
    export_onnx(
        wrapper, image, f"{args.output_dir}/onnx", dynamic_axes={"input": {0: "batch", 2: "patch"}}
    )
=======
    export_onnx(wrapper,
                image,
...
<<<<<<< HEAD
            ...
            # trained with TP
            mp0_weights = torch.load(
                tar.extractfile("./mp_rank_00/model_weights.ckpt"), map_location=args.device
            )
=======
...
<<<<<<< HEAD
    joined_path = os.path.join(os.path.dirname(args.model_path), os.path.basename(vision_path))
    if os.path.isdir(joined_path):
        vision_path = joined_path
    encoder = AutoModel.from_pretrained(
        vision_path, torch_dtype=torch.bfloat16, trust_remote_code=True
    )
=======
...
<<<<<<< HEAD
    ...
    wrapper = VisionEncoderWrapper(vision_encoder, vision_connector).to(args.device, dtype)
    image_size = hf_config.vision_config.image_size
...
<<<<<<< HEAD
    ...
    dummy_video = torch.empty(
        1, num_frames, 3, image_size, image_size, dtype=dtype, device=args.device
    )  # dummy image
...
<<<<<<< HEAD
    model = AutoModelForVision2Seq.from_pretrained(args.model_path, torch_dtype=torch.float16)
    wrapper = VisionEncoderWrapper(
        model.vision_model.to(args.device), model.image_to_text_projection.to(args.device)
    )
...
<<<<<<< HEAD
    processor = AutoProcessor.from_pretrained(args.model_path, trust_remote_code=True, num_crops=16)
    raw_image = Image.new("RGB", [10, 10])  # dummy image
    image = processor(text="<|image_1|>\ndummy", images=raw_image, return_tensors="pt")[
        "pixel_values"
    ].to(args.device, torch.float16)
...
<<<<<<< HEAD
    to_json_file(config_args, os.path.join(args.output_dir, "vision", "config.json"))
    to_json_file(config_args, os.path.join(args.output_dir, "audio", "config.json"))
=======
...
<<<<<<< HEAD
    ...
    audio_len = int((audio_feature_size * audio_compression_rate + 2) * audio_sampling_rate / 100)
    raw_audio = (np.zeros(audio_len), audio_sampling_rate)  # dummy audio

    inputs = processor(
        text="<|image_1|><|audio_1|>\ndummy",
        images=[raw_image],
        audios=[raw_audio],
        return_tensors="pt",
    )
    ...
    img_attention_mask = inputs["image_attention_mask"].to(args.device, torch.bool)
    img_embeds = img_embeds.flatten(0, 1)  # (2, 3, 448, 448)
    img_attention_mask = img_attention_mask.flatten(0, 1)  # (2, 32, 32)

    aud_embeds = inputs["input_audio_embeds"].to(args.device, torch.float16)  # (1, 4000, 80)
    aud_len, aud_dim = aud_embeds.shape[1:]
    aud_embeds = torch.cat([aud_embeds, aud_embeds.new_zeros(1, 4000 - aud_len, aud_dim)], dim=1)
    aud_attention_mask = torch.ones(1, aud_embeds.shape[1]).to(args.device, torch.bool)
...
<<<<<<< HEAD
            features = self.vision_model.get_img_features(img_embeds, attention_mask)
...
<<<<<<< HEAD
    ...
    build_trt_engine(
        args.model_type,
        input_sizes=[
            [list(img_embeds.shape[1:]) for _ in range(3)],
...
<<<<<<< HEAD
    ...
    build_trt_engine(
        args.model_type,
        input_sizes=[
            [list(aud_embeds.shape[1:]) for _ in range(3)],
            [list(aud_attention_mask.shape[1:]) for _ in range(3)],
        ],
        onnx_dir=onnx_dir,
        engine_dir=f"{args.output_dir}/{part_name}",
        max_batch_size=args.max_batch_size,
        engine_name="audio_encoder.engine",
        dtype=torch.float16,
    )
...
<<<<<<< HEAD
    ...
    wrapper = MLLaMAVisionWrapper(model.vision_model, model.multi_modal_projector)
    model_dtype = model.dtype
    image = Image.new("RGB", [2048, 2688])  # dummy image
    inputs = processor(images=image, return_tensors="pt").to(model_dtype).to(model.device)
=======
...
<<<<<<< HEAD
    ...
    if "InternVL2-26B" in args.model_path:
        image_processor = AutoProcessor.from_pretrained("OpenGVLab/InternViT-6B-448px-V1-5")
    else:
...
<<<<<<< HEAD
            ...
            vit_embeds_px = self.pixel_shuffle(vit_embeds, scale_factor=self.downsample_ratio)
            vit_embeds_px = vit_embeds_px.reshape(
                vit_embeds_px.shape[0], -1, vit_embeds_px.shape[-1]
            )
            vit_embeds_mlp = self.mlp1(vit_embeds_px)
            return vit_embeds_mlp

    model = AutoModelForCausalLM.from_pretrained(
        args.model_path, torch_dtype=torch.float16, trust_remote_code=True, use_flash_attn=False
    ).to(args.device)
    max_num_crops = model.config.max_dynamic_patch
    wrapper = InternvlVisionWrapper(model, model.config.downsample_ratio, model.config.select_layer)
...
<<<<<<< HEAD
    ...
    model = Qwen2VLForConditionalGeneration.from_pretrained(
        args.model_path, torch_dtype=torch.float32, device_map="cpu", attn_implementation="eager"
    )
    hf_config = AutoConfig.from_pretrained(args.model_path)
    qwen2_vl_dim = (
        hf_config.vision_config.in_chans
        * hf_config.vision_config.patch_size
        * hf_config.vision_config.patch_size
        * hf_config.vision_config.temporal_patch_size
    )
    processor = AutoProcessor.from_pretrained(args.model_path)
    messages = [
        {
...
<<<<<<< HEAD
        def __init__(self, config, attn_implementation: str = "eager") -> None:
            super().__init__(config)
            self.attn = VisionAttentionOpt(config.embed_dim, num_heads=config.num_heads)
...
<<<<<<< HEAD
    ...
    export_onnx(
        wrapper,
        (image, rotary_pos_emb, attention_mask),
        f"{args.output_dir}/onnx",
...
<<<<<<< HEAD
    ...
    audio = torch.randn(1, args.num_mul_bins, args.max_mel_seq_len, device=args.device)
...
<<<<<<< HEAD
    wrapper = AudioEncoderWrapper(model.audio_tower, model.multi_modal_projector)
=======
...
<<<<<<< HEAD
    ...
    build_trt_engine(
        ...
        args.output_dir,
        args.max_batch_size,
        model_params={"num_mul_bins": args.num_mul_bins, "max_mel_seq_len": args.max_mel_seq_len},
    )


def build_higgs_audio_engine(args):
    """Build TensorRT engine for Higgs Audio TTS model."""
    from tensorrt_llm.models.higgs_audio.model import load_higgs_audio_tokenizer

    audio_tokenizer = load_higgs_audio_tokenizer(args.model_path)
    ...
    class AudioEncoderWrapper(torch.nn.Module):
        ...
        def forward(self, x, mask):
            audio_outputs = self.audio_tokenizer.encode(x, sr=24000)
            audio_features = audio_outputs.last_hidden_state
            return audio_features

    wrapper = AudioEncoderWrapper(audio_tokenizer)
    wrapper.eval().to(args.device)
    del audio_tokenizer  # To save memory
...
def build_pixtral_engine(args):
    ...
    raw_image = Image.new(
<<<<<<< HEAD
        "RGB", [vision_config.image_size, vision_config.image_size]
    )  # dummy image
=======
        'RGB',
...
<<<<<<< HEAD
    ...
    from transformers.models.pixtral.modeling_pixtral import PixtralAttention, PixtralVisionModel
    ...
    @torch.no_grad
    def attn_forward(
        self, hidden_states, attention_mask, position_embeddings, output_attentions=False
    ):
=======
...
<<<<<<< HEAD
    model = Mistral3ForConditionalGeneration.from_pretrained(args.model_path, torch_dtype="auto")
=======
...
<<<<<<< HEAD
    ...
    build_trt_engine(
        args.model_type,
        input_sizes=[
            [list(pixel_values.shape[1:]) for _ in range(3)],
