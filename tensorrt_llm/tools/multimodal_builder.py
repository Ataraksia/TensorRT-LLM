...
def build_eclair_engine(args):
    class RadioWithNeck(torch.nn.Module):
        def __init__(self):
            ...
            try:
                self.model_encoder = torch.hub.load(
                    "NVlabs/RADIO", "radio_model", version="radio_v2.5-h"
                )
            except Exception as e:
                raise RuntimeError(f"Failed to load RADIO model from torch.hub: {e}")
            ...
            self.conv2 = torch.nn.Conv2d(
                1024, 1024, kernel_size=(1, 4), stride=(1, 4), padding=0, bias=False
            )
    ...
    image = torch.randn((1, 3, 2048, 1648), device=args.device, dtype=torch.bfloat16)
    export_onnx(wrapper, image, f"{args.output_dir}/onnx")
    build_trt_engine(
        args.model_type,
        [image.shape[1], image.shape[2], image.shape[3]],  # [3, H, W]
...
def build_higgs_audio_engine(args):
    ...
    from boson_multimodal import HiggsAudioModel  # noqa: F403

    model = HiggsAudioModel.from_pretrained(args.model_path, torch_dtype=torch.float16)
