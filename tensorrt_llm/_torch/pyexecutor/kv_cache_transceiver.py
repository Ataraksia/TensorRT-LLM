...
CacheTransBufferManagerCpp = (
    tensorrt_llm.bindings.internal.batch_manager.CacheTransBufferManager
)
...
def create_kv_cache_transceiver(
    ...
    if cache_transceiver_config.backend == BackendTypeCpp.MPI:
        logger.warning("MPI CacheTransceiver is deprecated, UCX or NIXL is recommended")
    ...
    return BindKvCacheTransceiver(
        mapping, dist, kv_cache_manager, attention_type, cache_transceiver_config
    )
...
class CacheTransBufferManager:
    ...
    def __init__(self, kv_cache_manager: KVCacheManager, max_num_tokens: int):
        self.impl = CacheTransBufferManagerCpp(kv_cache_manager.impl, max_num_tokens)

    @staticmethod
    def pre_alloc_buffer_size(
        kv_cache_size_per_token: int, cache_transceiver_config: CacheTransceiverConfig
    ):
