...
class QuantConfig:
    ...
    def _requires_calibration(self):
        return (
            ...
            in (
                set(QUANT_ALGO_LIST)
                - {QuantAlgo.W8A16, QuantAlgo.W4A16, QuantAlgo.FP8_PER_CHANNEL_PER_TOKEN}
            )
        ...
        if self.exclude_modules is not None:
            for exclude_module in self.exclude_modules:
                if exclude_module == module_name or (
                    exclude_module.endswith("*") and module_name.startswith(exclude_module[:-1])
                ):
    ...
    def _get_modelopt_qformat(self):
        ...
        assert self.quant_algo != QuantAlgo.MIXED_PRECISION, (
            "We don't support mixed precision in QuantConfig"
        )
        if self.quant_algo is not None:
            assert self.quant_algo in algo_to_modelopt_map, (
    ...
    def _get_modelopt_kv_cache_dtype(self):
        ...
        if self.kv_cache_quant_algo is not None:
            assert self.kv_cache_quant_algo in algo_to_modelopt_map, (
...
class LayerQuantConfig(QuantConfig):
        ...
        **kwargs,
        ...
        for name, layer_config in self.quantized_layers.items():
            self.auto_quant_mode.update(
                {
                    name: QuantMode.from_quant_algo(
        ...
        for key in kwargs:
            logger.warning(f"Warning: Unrecognized parameter '{key}' with value '{kwargs[key]}'")
    ...
    def _get_modelopt_qformat(self):
        ...
        autoq_format = ",".join([algo_to_modelopt_map[item] for item in self.auto_quant_list])
        return autoq_format
...
class PretrainedConfig:
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(position_embedding_type, str):
            position_embedding_type = PositionEmbeddingType.from_string(position_embedding_type)
        ...
        for key, value in kwargs.items():
            try:
                ...
                logger.warning(f"Implicitly setting {self.__class__.__name__}.{key} = {value}")
            except AttributeError as err:
                raise err

    @staticmethod
    def create_runtime_defaults(defaults: "RuntimeDefaultsIn" = None) -> Optional[RuntimeDefaults]:
    ...
    def from_json_file(cls, config_file: str):
        ...
        if obj.quantization.quant_algo == QuantAlgo.MIXED_PRECISION:
            try:
                layer_config_path = str(config_file).replace("config.json", "quant_cfg.json")
            ...
            for layer_name in list(config["quantized_layers"].keys()):
                ...
                if "mlp.fc" in layer_name or "mlp.proj" in layer_name:
                    ...
                    if moe_name not in config["quantized_layers"]:
                        ...
                        assert quant_cfg == config["quantized_layers"][moe_name], (
...
class PretrainedModel(Module, GenerationMixin, TopModelMixin, metaclass=PostInitCaller):
    ...
    def __post_init__(self):
        ...
        optimize_model(self, use_parallel_embedding=self.config.use_parallel_embedding)
    ...
    def from_checkpoint(
        ...
        *,
        preprocess_weights_hook: Optional[Callable[[Dict[str, Tensor]], Dict[str, Tensor]]] = None,
    ):
        if config is None:
            config = PretrainedConfig.from_json_file(os.path.join(ckpt_dir, "config.json"))
        ...
        weights = preprocess_weights(weights, config, from_pruned=is_checkpoint_pruned)
        model = cls(config)
    ...
    def prepare_inputs(
        ...
        result = {
            ...
            "kv_cache_params": KeyValueCacheParams(
                ...
                host_kv_cache_block_offsets=model_inputs["host_kv_cache_block_offsets"],
                host_kv_cache_pool_pointers=model_inputs["host_kv_cache_pool_pointers"],
                host_kv_cache_pool_mapping=model_inputs["host_kv_cache_pool_mapping"],
        ...
        if prompt_embedding_table_size > 0:
            result["prompt_embedding_table"] = model_inputs["prompt_embedding_table"]
        ...
        if model_inputs["spec_decoding_params"] is not None:
            result["spec_decoding_params"] = model_inputs["spec_decoding_params"]
    ...
    def quantize(
        ...
        **kwargs,
        ...
        if config.mapping.moe_ep_size > 1:
            raise NotImplementedError("Quantization for expert parallelism is not supported")
        ...
        if self.config.mapping.is_last_pp_rank():
            ...
            if is_gemma_2_cg or is_gemma_3_cg:
                softcap = self.config.get_config_group(
                    Gemma2ConfigGroup if not is_gemma_3_cg else Gemma3ConfigGroup
...
def fuse_gate_mlp(
    ...
    for name, mlp, layer in model.named_modules_with_parent():
        if isinstance(mlp, GatedMLP):
            ...
            if (
                isinstance(model.config.quantization.exclude_modules, list)
            ...
            if layer_quant_algo == QuantAlgo.FP8:
                ...
                if (
                    ...
                    fused_layer.fused_fc.weight = Parameter(
                        shape=(fused_layer.fused_fc.in_features, fused_layer.fused_fc.out_features),
                        dtype="fp8",
                    )
                    fused_layer.fused_fc.weight.value = fused_weight.view(
                        fused_layer.fused_fc.in_features, fused_layer.fused_fc.out_features
                    )
                else:
...
def unfuse_qkv_gemm(model: PretrainedModel) -> PretrainedModel:
    ...
    for name, layer in model.named_modules():
        if isinstance(layer, Attention) and not layer.cross_attention:
            ...
            qkv_params["strict_dtype"] = qkv_params.get("strict_dtype") is not None
            q = ColumnLinear(
                **{
                    **qkv_params,
            ...
            if layer.qkv.bias is not None and layer.qkv.bias.is_inited():
                ...
                biases = np.split(
                    ...
                    [
                        ...
                        qkv_bias.shape[out_dim] * (q.out_features + k.out_features) // out_features,
                    ],
                    axis=out_dim,
                )
...
def set_prompt_tuning(model: PretrainedModel) -> PretrainedModel:
    ...
    for name, embedding, parent in model.named_modules_with_parent():
        ...
        if layer_name == "vocab_embedding" and isinstance(embedding, Embedding):
            ptuning_embedding = PromptTuningEmbedding(**get_init_params(embedding))
...
def add_lora(
    model: PretrainedModel, max_lora_rank: Optional[int], with_dora: bool = False
) -> PretrainedModel:
    ...
    for name, layer in model.named_modules():
        ...
        if isinstance(layer, (MLP, FusedGatedMLP)):
            if max_rank is None:
                max_rank = min(layer.hidden_size, layer.ffn_hidden_size // layer.tp_size)
            layer.lora = Lora(
                in_hidden_size=layer.hidden_size,
                out_hidden_sizes=[
            ...
            if isinstance(layer, FusedGatedMLP):
                layer.fused_gate_up_lora = Lora(
                    in_hidden_size=layer.hidden_size,
                    out_hidden_sizes=[layer.ffn_hidden_size * 2 // layer.tp_size],
                    max_low_rank=max_rank,
                )
...
def set_fp8_context_fhma(model: PretrainedModel) -> PretrainedModel:
    for name, layer in model.named_modules():
        if isinstance(layer, Attention) and hasattr(layer.dense, "activation_scaling_factor"):
            ...
            scale = [1.0] / layer.dense.activation_global_scaling_factor.raw_value
            layer.attention_output_orig_quant_scale = Parameter(
                value=scale.astype(np.float32), dtype="float32"
            )
...
def set_fuse_fp4_quant(model: PretrainedModel) -> PretrainedModel:
    for name, layer in model.named_modules():
        if isinstance(layer, Attention) and hasattr(
            ...
            scale = [1.0] / layer.dense.activation_global_scaling_factor.raw_value
            layer.attention_output_sf_scale = Parameter(
                value=scale.astype(np.float32), dtype="float32"
            )
...
def optimize_model(
    ...
    if use_fused_mlp:
        model = fuse_gate_mlp(model, gemm_swiglu_plugin_dtype, low_latency_gemm_swiglu_plugin_dtype)
    if use_fused_rg_lru:
...
def optimize_cross_qkv(model):
    ...
    for name, attn, layer in model.named_modules_with_parent():
        if (
            isinstance(attn, Attention)
            and attn.cross_attention
            and (type(attn.qkv) == ColumnLinear or type(attn.qkv) == FP8Linear)
        ):
            old_qkv = attn.qkv
            ...
            if (
                ...
                new_kv.weight.value = kv_weight.reshape(
                    [2 * attn.num_attention_kv_heads * attn.attention_head_size, attn.hidden_size]
                )
            elif (
                old_qkv_weight_value.shape
                == np.asarray(
                ...
                new_kv.weight.value = kv_weight.reshape(
                    [attn.hidden_size, 2 * attn.num_attention_kv_heads * attn.attention_head_size]
                )
            else:
                assert False
...
def preprocess_perlayer_weights(weights, model_config, quant_algo, from_pruned=False):
    ...
    if quant_algo == QuantAlgo.W4A8_AWQ or quant_algo == QuantAlgo.W4A16_AWQ:
        ...
        for name, param in weights.items():
            ...
            if name.endswith("weight") and param.dtype == torch.int8:
                ...
                weights[name] = preprocessor(
                    param.transpose(-1, -2), torch.quint4x2, activation_type
                ).view(dtype)
            if name.endswith("weights_scaling_factor"):
                weights[name] = (
                    param.transpose(-1, -2).contiguous().to(str_dtype_to_torch(model_config.dtype))
                )
            ...
            if model_config.mapping.tp_rank > 0:
                if name.endswith("attention.dense.bias") or name.endswith("mlp.proj.bias"):
            ...
            for name in list(weights):
                if name.endswith("weights_scaling_factor"):
                    activation_scaling_factor = weights.pop(
                        name.replace("weights_scaling_factor", "activation_scaling_factor")
                    )
                    weights_scaling_factor_2 = weights.pop(
                        name.replace("weights_scaling_factor", "weights_scaling_factor_2")
                    )
                    weights[name] /= weights_scaling_factor_2
                    weights[name] = (
                        weights[name].to(torch.float16).view(str_dtype_to_torch(model_config.dtype))
                    )
                    weights[name.replace("weights_scaling_factor", "prequant_scaling_factor")] /= (
                        activation_scaling_factor
                    )
                    weights[name.replace("weights_scaling_factor", "alpha")] = (
                        activation_scaling_factor * weights_scaling_factor_2
                    )
                    weights[name.replace("weights_scaling_factor", "activation_scaling_factor")] = (
                        activation_scaling_factor
                    )
    ...
    # FP8
        ...
        if "lm_head.weight" in weights:
            assert weights["lm_head.weight"].dtype == str_dtype_to_torch(model_config.dtype)
            weights.pop("lm_head.weights_scaling_factor", None)
            weights.pop("lm_head.activation_scaling_factor", None)
    # FP4
        ...
        for name in list(weights):
            if name.endswith("weights_scaling_factor"):
                ...
                new_name = name.replace("weights_scaling_factor", "weights_block_scaling_factor")
            ...
            if name.endswith("weights_scaling_factor_2"):
                new_name = name.replace("weights_scaling_factor_2", "weights_global_scaling_factor")
                weights[new_name] = weights[name]
                weights.pop(name)
            if name.endswith("activation_scaling_factor"):
                new_name = name.replace(
                    "activation_scaling_factor", "activation_global_scaling_factor"
                )
                weights[new_name] = weights[name]
                weights.pop(name)
        for name in list(weights):
            if name.endswith("weights_global_scaling_factor"):
                ...
                act_global_sf = weights[
                    name.replace(
                        "weights_global_scaling_factor", "activation_global_scaling_factor"
                    )
                ]
                weights[name.replace("weights_global_scaling_factor", "alpha")] = (
                    act_global_sf * weight_global_sf
                )
    elif quant_algo in [QuantAlgo.W4A16, QuantAlgo.W8A16]:
        weights = weight_only_quantize_dict(
            weights=weights, quant_algo=quant_algo, exclude_modules=exclude_modules, plugin=True
        )


def preprocess_weights(
    weights: Dict[str, torch.Tensor], model_config: PretrainedConfig, from_pruned=False
) -> None:
    ...
    def process_kv_scaling_factor(weights: Dict[str, torch.Tensor]):
        ...
        for name, param in weights.items():
            if name.endswith(".k_cache_scaling_factor"):
                v_name = name.replace("k_cache_scaling_factor", "v_cache_scaling_factor")
                assert v_name in weights, f"{v_name} not found"
                kv_name = name.replace("k_cache_scaling_factor", "kv_cache_scaling_factor")
        ...
        for name, param in weights.items():
            if name.endswith(".kv_cache_scaling_factor"):
                rcp_name = name.replace("kv_cache_scaling_factor", "kv_cache_rcp_scaling_factor")
    ...
    for base_name, layer_weights in per_layer_weights.items():
        ...
        preprocess_perlayer_weights(layer_weights, model_config, layer_quant_algo, from_pruned)
        new_weights.update(layer_weights)
...
def get_kv_cache_type_from_legacy(use_cache: bool, paged_kv_cache: bool) -> KVCacheType:
    if use_cache:
        if paged_kv_cache:
...
def save_config(config: PretrainedConfig, *, output_dir: str, log: bool) -> None:
