...
import numpy as np
import torch
from torch.nn import AvgPool1d, ModuleList
...
class HiggsAudioEncoderLayer(Module):
    ...
    def __init__(self, config, layer_idx: int = 0):
        ...
        # Layer normalization
        self.self_attn_layer_norm = LayerNorm(normalized_shape=self.embed_dim, dtype=config.dtype)
        ...
        self.final_layer_norm = LayerNorm(normalized_shape=self.embed_dim, dtype=config.dtype)
    ...
    def __init__(self, config: HiggsAudioConfig):
        ...
        if not config.skip_audio_tower:
            ...
            self.layers = ModuleList(
                [HiggsAudioEncoderLayer(config, layer_idx) for layer_idx in range(self.num_layers)]
            )

            # Average pooling layer for sequence reduction (like original Transformers)
            self.avg_pooler = AvgPool1d(kernel_size=2, stride=2, padding=0)

            # Final layer normalization
            self.layer_norm = LayerNorm(normalized_shape=self.embed_dim, dtype=config.dtype)
        else:
...
class HiggsAudioEncoderProjector(Module):
    ...
    def prepare_multimodal_inputs(
        ...
        if audio_features is not None and audio_mask is not None:
            ...
            combined_embeds = where(audio_mask.unsqueeze(-1), audio_features, text_embeds)
        else:
            # Text-only mode
    ...
    def multimodal_forward(
        ...
        mm_inputs = self.prepare_multimodal_inputs(
            input_ids=input_ids, audio_features=audio_features, audio_mask=audio_mask
        )
        ...
        if audio_mask is not None and audio_mask.any():
            ...
            audio_hidden = where(audio_mask.unsqueeze(-1), hidden_states, constant(0.0))
...
class HiggsAudioDecoderProjector(Module):
    ...
    def forward(
        self, hidden_states: Tensor, audio_out_mask: Optional[Tensor] = None
    ) -> Tuple[Tensor, Tensor]:
...
class HiggsAudioDecoderLayer(Module):
    ...
    def forward(
        ...
        next_layer_input_layernorm_args: Optional[Any] = None,
...
class HiggsAudioDualFFNDecoderLayer(Module):
    ...
    def _validate_config(self, config: HiggsAudioConfig, layer_idx: int) -> None:
        ...
        if not hasattr(config, "intermediate_size") or config.intermediate_size <= 0:
            raise ValueError(
                f"Invalid intermediate_size: {getattr(config, 'intermediate_size', None)}"
            )
    ...
    def _initialize_attention(self, config: HiggsAudioConfig) -> None:
        ...
        self.attention = Attention(
            ...
            attention_head_size=getattr(
                config, "head_size", config.hidden_size // config.num_attention_heads
            ),
            num_attention_heads=config.num_attention_heads,
    ...
    def _dual_ffn_forward(
        self, hidden_states: Tensor, lora_layer_params: Optional[Any] = None
    ) -> Tensor:
    ...
    def forward(
        ...
        if self.use_audio_attention and audio_out_mask is not None:
            if audio_out_mask.any():  # Only if we have audio tokens
                ...
                audio_attn_out = self.audio_attention(
                    audio_norm,
                    attention_mask=attention_mask,
...
class HiggsAudioModel(Module):
    ...
    def __init__(self, config: HiggsAudioConfig) -> None:
        ...
        if self.mapping.is_first_pp_rank():
            self.vocab_embedding = EmbeddingCls(
                ...
                dtype=config.dtype,
                tp_size=self.mapping.tp_size
                if getattr(config, "use_parallel_embedding", False)
                else 1,
                tp_group=self.mapping.tp_group
                if getattr(config, "use_parallel_embedding", False)
                else None,
    ...
    def _initialize_decoder_layers(self, config: HiggsAudioConfig) -> None:
        ...
        if not hasattr(config, "num_hidden_layers") or config.num_hidden_layers <= 0:
            raise ValueError(
                f"Invalid num_hidden_layers: {getattr(config, 'num_hidden_layers', None)}"
            )
    ...
    def forward(
        ...
        audio_token_mask: Optional[Tensor] = None,
    ) -> Union[Tensor, Tuple[Tensor, Tuple[Any, ...]]]:
        ...
        audio_mask_for_layers = (
            self.audio_out_mask if self.audio_out_mask is not None else audio_token_mask
        )
...
class HiggsAudioForCausalLM(DecoderModelForCausalLM, TopModelMixin):
    ...
    def sample_audio_tokens_with_delay_pattern(
        ...
        vocab_per_cb = self.audio_codebook_size + 2
        ...
        if num_remaining_delays is None:
            last_eos_idx = None
    ...
    def prepare_inputs(self, *args, **kwargs):
        ...
        if hasattr(self, "_generation_state"):
            if self._generation_state.get("mode") == GenerationMode.AUDIO_IN_PROGRESS:
                ...
                inputs["num_remaining_delays"] = self._generation_state.get(
                    "num_remaining_delays", None
                )
    ...
    def from_hugging_face(
        ...
        **kwargs: Any,
        ...
        Example:
            ...
            ...     "path/to/higgs-audio-model", dtype="float16", mapping=tp_mapping
            ... )
        """
        ...
        **kwargs,
        ...
        for step in range(max_new_tokens):
            ...
            if current_mode == "audio" and "audio" in logits:
                ...
                if "text" in logits:
                    ...
                    next_tokens = self._sample_tokens(
                        text_logits[:, -1, :], temperature=temperature, top_k=top_k, top_p=top_p
                    )
            ...
            next_tokens_expanded = (
                next_tokens.unsqueeze(1) if len(next_tokens.shape) == 1 else next_tokens
            )
