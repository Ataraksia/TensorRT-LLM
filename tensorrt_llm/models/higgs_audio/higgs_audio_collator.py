...
from .chatml_dataset import ChatMLDatasetSample
from .utils import build_delay_pattern_mask
...
class HiggsAudioSampleCollator:
    ...
    def __call__(self, batch: List[ChatMLDatasetSample]):
        ...
        if self.encode_whisper_embed:
            ...
            for i in range(len(batch)):
                ...
                for idx, audio_idx in enumerate(audio_in_indices):
                    ...
                    # Ensure wv is on CPU for librosa processing
                    ...
                    if sr != self.whisper_processor.feature_extractor.sampling_rate:
                        ...
                        resampled_wv = wv
                    wv = torch.tensor(resampled_wv, device='cpu')
        ...
        for i in range(len(processed_batch)):
            ...
            audio_ids = torch.ones_like(processed_batch[i].input_ids, dtype=torch.int32)
            audio_ids[audio_in_mask ^ audio_out_mask] = (torch.cumsum(audio_ids[audio_in_mask ^ audio_out_mask], 0) - 1).to(torch.int32)
